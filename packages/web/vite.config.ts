...
const pwaDevEnabled = process.env.ORBIT_DISABLE_PWA_DEV !== '1';
...
export default defineConfig({
  ...
  resolve: {
    alias: [
      { find: '@opencode-ai/sdk/v2', replacement: new URL('.', import.meta.resolve('@opencode-ai/sdk/v2/client')).pathname + 'client.js' },
      { find: '@orbit/ui', replacement: path.resolve(__dirname, '../ui/src') },
  ...
  server: {
    ...
    proxy: {
      '/auth': {
        target: `http://127.0.0.1:${process.env.ORBIT_PORT || 3001}`,
        changeOrigin: true,
      },
      '/health': {
        target: `http://127.0.0.1:${process.env.ORBIT_PORT || 3001}`,
