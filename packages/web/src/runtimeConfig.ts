import { getRuntimeExtraHeadersSync, refreshLocalRuntimeUrlAuthToken, refreshRuntimeUrlAuthToken, setRuntimeBearerToken, setRuntimeExtraHeaders } from '@orbit/ui/lib/runtime-auth';
import { installRuntimeFetchBridge } from '@orbit/ui/lib/runtime-fetch';
...
import { getInjectedBootOutcome } from '@orbit/ui/lib/desktopBoot';
import { configureRuntimeUrlResolver } from '@orbit/ui/lib/runtime-url';
import type { EmbeddedSessionRuntimeBootstrap } from '@orbit/ui/components/layout/contextPanelEmbeddedChat';
...
declare global {
  interface Window {
    __ORBIT_API_BASE_URL__?: string;
    __ORBIT_CLIENT_TOKEN__?: string;
    __ORBIT_RUNTIME_HEADERS__?: Record<string, string>;
    __ORBIT_LOCAL_ORIGIN__?: string;
    __ORBIT_RELAY_HOST_ID__?: string;
  }
}
...
export const readRuntimeBootstrapConfig = (): EmbeddedSessionRuntimeBootstrap => {
  ...
  return {
    apiBaseUrl: readString(window.__ORBIT_API_BASE_URL__),
    clientToken: readString(window.__ORBIT_CLIENT_TOKEN__),
    localOrigin: readString(window.__ORBIT_LOCAL_ORIGIN__),
    runtimeHeaders: window.__ORBIT_RUNTIME_HEADERS__,
    relayHostId: readString(window.__ORBIT_RELAY_HOST_ID__),
  };
};
