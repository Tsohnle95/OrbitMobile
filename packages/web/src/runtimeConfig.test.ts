...
vi.mock('@orbit/ui/lib/runtime-auth', () => ({
...
vi.mock('@orbit/ui/lib/runtime-fetch', () => ({ installRuntimeFetchBridge: vi.fn() }));
vi.mock('@orbit/ui/lib/runtime-switch', () => ({
  getRuntimeApiBaseUrl: vi.fn(() => ''),
  getRuntimeKey: vi.fn(() => 'local'),
  initializeRuntimeEndpoint: vi.fn(),
  switchRuntimeEndpoint: vi.fn(),
}));
vi.mock('@orbit/ui/lib/desktopRelayRestore', () => ({ restoreDesktopRelayRuntime: vi.fn(() => Promise.resolve()) }));
vi.mock('@orbit/ui/lib/runtime-url', () => ({ configureRuntimeUrlResolver: vi.fn(() => ({})) }));
vi.mock('@orbit/ui/lib/opencode/client', () => ({ opencodeClient: { reconnectToRuntimeBaseUrl: vi.fn() } }));
vi.mock('./api', () => ({ createWebAPIs: vi.fn() }));

import { setRuntimeBearerToken, setRuntimeExtraHeaders } from '@orbit/ui/lib/runtime-auth';
import { initializeRuntimeEndpoint, switchRuntimeEndpoint } from '@orbit/ui/lib/runtime-switch';
import { restoreDesktopRelayRuntime } from '@orbit/ui/lib/desktopRelayRestore';
import { opencodeClient } from '@orbit/ui/lib/opencode/client';
...
const makeWindow = (search = ''): Record<string, unknown> => {
  const value: Record<string, unknown> = {
    location: { origin: 'orbit-ui://app', search },
    setTimeout: vi.fn(() => 1),
  };
  value.parent = value;
  return value;
...
describe('readRuntimeBootstrapConfig', () => {
  test('reads the runtime injected into the current window', () => {
    ...
    current.__ORBIT_API_BASE_URL__ = ' https://remote.example.com ';
    current.__ORBIT_CLIENT_TOKEN__ = ' remote-token ';
    current.__ORBIT_LOCAL_ORIGIN__ = ' http://127.0.0.1:3000 ';
    current.__ORBIT_RUNTIME_HEADERS__ = { 'x-orbit-relay': 'relay-value' };
    current.__ORBIT_RELAY_HOST_ID__ = ' remote-host ';
    ...
    expect(readRuntimeBootstrapConfig()).toEqual({
      ...
      runtimeHeaders: { 'x-orbit-relay': 'relay-value' },
  ...
  test('does not read runtime credentials directly from a parent window', () => {
    ...
    parent.__ORBIT_API_BASE_URL__ = 'https://remote.example.com';
    parent.__ORBIT_CLIENT_TOKEN__ = 'remote-token';
...
describe('createConfiguredWebAPIs', () => {
  ...
  test('uses the configured desktop host id across changing SSH tunnel URLs', () => {
    const current = makeWindow();
    current.__ORBIT_DESKTOP_BOOT_OUTCOME__ = {
      target: 'remote',
      status: 'ok',
      hostId: 'ssh-castle',
    ...
    current.__ORBIT_API_BASE_URL__ = 'http://127.0.0.1:62545';
    current.__ORBIT_LOCAL_ORIGIN__ = 'http://127.0.0.1:3901';
    installWindow(current);
