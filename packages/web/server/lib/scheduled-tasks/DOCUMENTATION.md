...
Server-owned scheduled task runtime and routes for Orbit-only automation.
...
- This module is Orbit feature logic; it is intentionally separate from OpenCode proxy/runtime internals.

## Cross-instance occurrence claiming

Multiple Orbit server processes can share the same on-disk project config
...
- `packages/web/server/lib/scheduled-tasks/routes.js`
  ...
  - Manual run endpoint
  - Orbit events SSE stream endpoint
...
- Registers:
  ...
  - `GET /api/orbit/scheduled-tasks/status`
  - `GET /api/orbit/events`
