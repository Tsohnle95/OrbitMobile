...
export const registerScheduledTaskRoutes = (app, dependencies) => {
  ...
  app.get('/api/orbit/scheduled-tasks/status', async (_req, res) => {
    try {
  ...
  app.get('/api/orbit/events', (req, res) => {
    ...
    res.orbitBrowserCapable = req.query?.browser === '1';

    const clients = getOrbitEventClients();
    clients.add(res);

    try {
      writeSseEvent(res, {
        type: 'orbit:event-stream-ready',
        properties: {
