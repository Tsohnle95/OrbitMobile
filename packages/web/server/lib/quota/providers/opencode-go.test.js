...
const previousDataDirectory = process.env.ORBIT_DATA_DIR;
const temporaryDataDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'orbit-opencode-go-'));
process.env.ORBIT_DATA_DIR = temporaryDataDirectory;
...
afterAll(() => {
  if (previousDataDirectory === undefined) delete process.env.ORBIT_DATA_DIR;
  else process.env.ORBIT_DATA_DIR = previousDataDirectory;
