...
export const resolveOpenCodeEnvConfig = (options = {}) => {
  ...
  const configuredOpenCodePort = (() => {
    const raw =
      env.OPENCODE_PORT ||
      env.ORBIT_OPENCODE_PORT ||
      env.ORBIT_INTERNAL_PORT;
    if (!raw) {
      return null;
    }
  ...
  const configuredOpenCodeHostname = (() => {
    const raw = env.ORBIT_OPENCODE_HOSTNAME;
    ...
    if (!trimmed) {
      logger.warn(
        `[config] Ignoring ORBIT_OPENCODE_HOSTNAME=${JSON.stringify(raw)}: empty after trimming`,
      );
      return '127.0.0.1';
    }
    if (!isValidOpenCodeHostname(trimmed)) {
      logger.error(
        `[config] Rejecting ORBIT_OPENCODE_HOSTNAME=${JSON.stringify(raw)}: `
