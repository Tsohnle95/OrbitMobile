...
describe('settings helpers', () => {
  ...
  it('includes transient desktop LAN access runtime status in desktop settings response', () => {
    const helpers = createTestHelpers();
    const previousRuntime = process.env.ORBIT_RUNTIME;
    const previousActive = process.env.ORBIT_DESKTOP_LAN_ACCESS_ACTIVE;
    const previousReason = process.env.ORBIT_DESKTOP_LAN_ACCESS_BLOCKED_REASON;
    try {
      process.env.ORBIT_RUNTIME = 'desktop';
      process.env.ORBIT_DESKTOP_LAN_ACCESS_ACTIVE = 'false';
      process.env.ORBIT_DESKTOP_LAN_ACCESS_BLOCKED_REASON = 'missing-password';
      ...
      if (typeof previousRuntime === 'string') process.env.ORBIT_RUNTIME = previousRuntime;
      else delete process.env.ORBIT_RUNTIME;
      if (typeof previousActive === 'string') process.env.ORBIT_DESKTOP_LAN_ACCESS_ACTIVE = previousActive;
      else delete process.env.ORBIT_DESKTOP_LAN_ACCESS_ACTIVE;
      if (typeof previousReason === 'string') process.env.ORBIT_DESKTOP_LAN_ACCESS_BLOCKED_REASON = previousReason;
      else delete process.env.ORBIT_DESKTOP_LAN_ACCESS_BLOCKED_REASON;
    }
  });
