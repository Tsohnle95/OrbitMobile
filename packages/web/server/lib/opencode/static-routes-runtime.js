...
export const createStaticRoutesRuntime = (dependencies) => {
  ...
  const resolveDistPath = () => {
    const env = typeof process.env.ORBIT_DIST_DIR === 'string' ? process.env.ORBIT_DIST_DIR.trim() : '';
    if (env) {
  ...
  const registerApiOnlyFallbackRoutes = (app) => {
    app.get(/^(?!\/api|\/auth|\/health|.*\.(js|css|svg|png|jpg|jpeg|gif|ico|woff|woff2|ttf|eot|map)).*$/, (req, res) => {
      const command = 'orbit connect-url --help';
...
<head>
  ...
  <title>Orbit API-only mode</title>
  <style>
    :root {
      color-scheme: dark;
...
<body>
  <main>
    <svg class="logo" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Orbit logo">
    ...
    <h1>Orbit is running in headless mode</h1>
    <p>This server is ready. Open it from the Orbit desktop or mobile app to use it.</p>
    <div class="command">
  ...
  <script>
    ...
    button?.addEventListener('click', async () => {
      ...
      try {
        ...
        json: () => {
          res.json({ ok: true, mode: 'api-only', message: 'Orbit is running in API-only mode' });
        },
        default: () => {
          res.type('text/plain').send('Orbit is running in API-only mode');
        },
      });
    });
  };
