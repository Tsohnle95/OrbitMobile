...
describe('OpenCode lifecycle', () => {
  ...
  it('strips AppImage ARGV0 from managed OpenCode launch env', async () => {
    ...
    process.env.ARGV0 = '/path/to/Orbit/Orbit-1.17.2-linux-x86_64.AppImage';
    const child = createMockChild();
    spawnMock.mockImplementationOnce(() => {
      queueMicrotask(() => {
        child.stdout.emit('data', 'opencode server listening on http://127.0.0.1:45678\n');
  ...
  it('adds managed Orbit tool environment without allowing it to replace launch invariants', async () => {
    const child = createMockChild();
    spawnMock.mockImplementationOnce(() => {
      queueMicrotask(() => {
        child.stdout.emit('data', 'opencode server listening on http://127.0.0.1:45678\n');
    ...
    const getManagedOpenCodeEnv = vi.fn(async () => ({
      ...
      ORBIT_AGENT_TOOL_TOKEN: 'ephemeral',
      PATH: '/untrusted/path',
    ...
    expect(options.env.ORBIT_AGENT_TOOL_TOKEN).toBe('ephemeral');
    expect(options.env.PATH).toBe('/home/user/.bun/bin:/usr/local/bin:/usr/bin');
    expect(options.env.OPENCODE_SERVER_PASSWORD).toBe('password');
