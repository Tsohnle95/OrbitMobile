...
export const registerOpenCodeRoutes = (app, dependencies) => {
  ...
  // needs ships inline. `orbit://focus/mcp-auth` raises the desktop app;
...
${desktopReturn ? `<a class="return" href="orbit://focus/mcp-auth">Return to Orbit</a>
<script>window.location.href = 'orbit://focus/mcp-auth';</script>` : ''}
</main>
</body>
</html>`;
      ...
      if (!capability.supported) {
        return res.status(409).json({
          ...
          code: capability.reason === 'bundled'
            ? 'OPENCODE_UPGRADE_MANAGED_BY_ORBIT'
            : 'OPENCODE_UPGRADE_UNSUPPORTED',
          error: capability.reason === 'bundled'
            ? 'OpenCode is bundled with Orbit Desktop and updates with the app.'
            : 'This OpenCode runtime cannot be upgraded by Orbit.',
        });
      }
  ...
  // Orbit UI session — the SPA route this path used to land on sits
  ...
  app.get('/mcp/oauth/callback', async (req, res) => {
    ...
    if (!context?.name) {
      return finish(400, {
        title: 'Authorization Failed',
        message: 'This authorization session has expired or is unknown to the running app. Return to Orbit and click Authorize again.',
      });
    }
    ...
    try {
      ...
      return finish(200, {
        ...
        message: 'You can close this tab and return to Orbit.',
      });
    } catch (error) {
      return finish(502, {
        title: 'Authorization Failed',
