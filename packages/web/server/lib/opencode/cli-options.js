export const parseServeCliOptions = ({
  ...
  const envPassword =
    env.ORBIT_UI_PASSWORD ||
    env.OPENCODE_UI_PASSWORD ||
    null;
  const envCfTunnel = env.ORBIT_TRY_CF_TUNNEL === 'true';
  const envTunnelProvider = env.ORBIT_TUNNEL_PROVIDER || undefined;
  const envTunnelMode = env.ORBIT_TUNNEL_MODE || undefined;
  const envTunnelConfigRaw = env.ORBIT_TUNNEL_CONFIG;
  const envTunnelConfig = typeof envTunnelConfigRaw === 'string'
    ? (envTunnelConfigRaw.trim().length > 0 ? envTunnelConfigRaw.trim() : null)
    : undefined;
  const envTunnelToken = env.ORBIT_TUNNEL_TOKEN || undefined;
  const envTunnelHostname = env.ORBIT_TUNNEL_HOSTNAME || undefined;
  const envApiOnly = env.ORBIT_API_ONLY === '1' || env.ORBIT_API_ONLY === 'true';
