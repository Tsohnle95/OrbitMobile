        ...
        try {
          ...
          if (typeof process.send === 'function') {
            if (!process.connected) {
              throw new Error('Orbit startup IPC channel disconnected before ready notification');
            }

            await new Promise((resolveReadyNotification, rejectReadyNotification) => {
              try {
                process.send({ type: 'orbit:ready', port: activePort }, (error) => {
                  if (error) {
          ...
          console.log(`Orbit server listening on ${bindHost}:${activePort}`);
