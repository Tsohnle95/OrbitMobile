...
function resolveSystemdServiceUnit(environment) {
  ...
  const configuredUnit = typeof environment.ORBIT_SYSTEMD_UNIT === 'string'
    ? environment.ORBIT_SYSTEMD_UNIT.trim()
    : '';
  const unit = configuredUnit || 'orbit.service';
...
export const registerOrbitRoutes = (app, dependencies) => {
  const {
    fs,
    path,
    process,
  ...
  app.get('/api/orbit/update-check', async (req, res) => {
    try {
  ...
  app.post('/api/orbit/update-install', async (_req, res) => {
    try {
      ...
      const instanceFilePath = path.join(orbitDataDir, 'run', `orbit-${currentPort}.json`);
      ...
      if (isForegroundService) {
        ...
        const updateJobName = `orbit-update-${Date.now()}`;
      ...
      let restartCmdFallback = `orbit serve --port ${storedOptions.port}`;
      ...
      const updateLogPath = path.join(orbitDataDir, 'update-install.log');
      const logPreamble = [
        '',
        `=== Orbit update ${new Date().toISOString()} ===`,
      ...
      res.json({
        ...
        setTimeout(() => {
          ...
          const script = isWindows
            ...
            ${updateCmd}
            if %ERRORLEVEL% EQU 0 (
              echo Update successful, restarting Orbit...
              ${restartCmd || 'echo Service manager will restart Orbit.'}
            ) else (
              echo Update failed
              exit /b 1
            )
  ...
  app.get('/api/orbit/models-metadata', async (_req, res) => {
    try {
