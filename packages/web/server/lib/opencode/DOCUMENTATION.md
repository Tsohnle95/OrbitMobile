...
- `packages/web/server/lib/opencode/bootstrap-runtime.js`: base app bootstrap runtime for status/auth/tts/notification/Orbit route wiring.
...
- `packages/web/server/lib/opencode/orbit-routes.js`: Orbit update and models metadata route registration.
...
- Returned API:
  ...
  - `interruptBusySessionsAfterRestart()`: settles every session whose authoritative status is `busy`/`retry` or whose activity phase is still busy, broadcasts `orbit:session-status` idle plus an OpenCode-shaped `session.error`, resets leftover activity/cooldowns, and returns the interrupted session IDs in stable order.
  - `dispose()`
...
Orbit tool injection. Managed launch env strips AppImage `ARGV0` before
...
- `registerSettingsUtilityRoutes(app, dependencies)`: registers small settings utility endpoints:
  ...
  - `POST /api/config/reload` — applies accumulated deferred OpenCode config changes. Managed OpenCode restarts and returns `requiresReload: true`. External OpenCode returns `requiresManualRestart: true` (changes are already on disk; the connected server must be restarted outside Orbit).
...
The pipeline binds the Orbit listener and publishes its active port
before starting managed OpenCode. The managed custom tool therefore receives
an authoritative loopback callback URL even when Orbit binds port `0`.

## Public exports (orbit-routes.js)
- `registerOrbitRoutes(app, dependencies)`: registers Orbit endpoints:
  - `GET /api/orbit/update-check`
  - `POST /api/orbit/update-install`
    - Foreground servers running under a systemd user unit queue installation in
      ...
      `ORBIT_SYSTEMD_UNIT` overrides the default `orbit.service`.
  - `GET /api/orbit/models-metadata`
  - `GET /api/zen/models`
...
- Rate limit config: `ORBIT_RATE_LIMIT_MAX_ATTEMPTS`, `ORBIT_RATE_LIMIT_NO_IP_MAX_ATTEMPTS` env vars.
