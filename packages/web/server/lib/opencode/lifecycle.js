...
const HEALTH_CHECK_TIMEOUT_MS = parsePositiveInt(process.env.ORBIT_OPENCODE_HEALTH_TIMEOUT_MS, 5000);
const HEALTH_CHECK_MAX_CONSECUTIVE_FAILURES = parsePositiveInt(
  process.env.ORBIT_OPENCODE_HEALTH_CONSECUTIVE_FAILURES,
  20
);
...
export const createOpenCodeLifecycleRuntime = (deps) => {
  ...
  const createManagedOpenCodeServerProcess = async ({ hostname, port, timeout, cwd, env: processEnv, shellEnvKeysCount = 0 }) => {
    ...
    const url = await new Promise((resolve, reject) => {
      ...
      const onExit = (code, signal) => {
        ...
        const appBundleHint = process.platform === 'darwin' && /\/OpenCode\.app\/Contents\/MacOS\/(?:OpenCode|opencode-cli)$/i.test(binary)
          ? ' The configured binary appears to point at the macOS desktop app bundle; Orbit needs the standalone opencode CLI.'
          : '';
    ...
    // actual host (Electron sets ORBIT_RUNTIME='desktop'; the standalone
    ...
    registerManagedProcess({
      ...
      port,
      binary,
      runtime: process.env.ORBIT_RUNTIME || 'web',
    });
  ...
  const refreshOpenCodeAfterConfigChange = async (reason, options = {}) => {
    ...
    // Orbit: restartOpenCode() only re-probes its health, so the freshly
