...
//     for self-hosters who set ORBIT_APNS_* and ORBIT_PUSH_RELAY_DISABLED=true.
...
const DEFAULT_BUNDLE_ID = 'com.orbit.mobile';
...
export const createApnsRuntime = (deps) => {
  ...
  const resolveApnsConfig = async () => {
    let keyId = trimmedEnv('ORBIT_APNS_KEY_ID');
    let teamId = trimmedEnv('ORBIT_APNS_TEAM_ID');
    let bundleId = trimmedEnv('ORBIT_APNS_BUNDLE_ID');
    let environment = (trimmedEnv('ORBIT_APNS_ENVIRONMENT') || '').toLowerCase();
    let p8 = normalizePem(process.env.ORBIT_APNS_P8 || '');

    const p8Path = trimmedEnv('ORBIT_APNS_P8_PATH');
    if (!p8 && p8Path) {
      try {
        p8 = (await fsPromises.readFile(p8Path, 'utf8')).trim();
      } catch (error) {
        console.warn('[APNs] Failed to read ORBIT_APNS_P8_PATH:', error?.message ?? error);
      }
    }
  ...
  // is the fallback for self-hosters who set ORBIT_APNS_* and disable the relay.
  const resolveRelayConfig = () => {
    if (trimmedEnv('ORBIT_PUSH_RELAY_DISABLED') === 'true') return null;
    const url = trimmedEnv('ORBIT_PUSH_RELAY_URL') || DEFAULT_RELAY_URL;
    const override = (trimmedEnv('ORBIT_APNS_ENVIRONMENT') || '').toLowerCase();
    return {
      url,
      registerUrl: url.replace(/\/send$/, '/register-token'),
      // Explicit ORBIT_APNS_ENVIRONMENT forces every send to that environment; when
  ...
  const sendViaRelay = async (deviceTokens, payload, relay, environment) => {
    ...
    const title = typeof payload?.title === 'string' && payload.title.length > 0 ? payload.title : 'Orbit';
    const { privateKey, publicJwk } = await getOrCreateRelayKeypair();
    const ts = Date.now();
  ...
  const sendViaDirectApns = async (tokenGroups, payload) => {
    ...
    if (!config) {
      if (!warnedUnconfigured) {
        ...
        console.warn(
          '[APNs] Relay disabled and no direct config; set ORBIT_APNS_KEY_ID / ORBIT_APNS_TEAM_ID / ORBIT_APNS_P8 for direct send.',
        );
      }
      return;
    }
