...
4. The **relay** (`orbit-website/apps/api`, Cloudflare Worker) verifies the signature +
...
- **Relay (default):** server has no Apple key; `ORBIT_PUSH_RELAY_URL` defaults to
  `https://api.openchamber.dev/v1/push/send` (register URL is derived as `…/register-token`).
- **Direct (fallback):** set `ORBIT_PUSH_RELAY_DISABLED=true` + `ORBIT_APNS_KEY_ID/
  TEAM_ID/P8` to sign+send from the server itself (HTTP/2 + ES256 JWT); no relay binding needed.

## Config

Server (`apns-runtime.js`):
- `ORBIT_PUSH_RELAY_URL` (default the public relay), `ORBIT_APNS_ENVIRONMENT`
...
- Direct fallback: `ORBIT_APNS_KEY_ID`, `ORBIT_APNS_TEAM_ID`, `ORBIT_APNS_P8`
  (or `_P8_PATH`), `ORBIT_APNS_BUNDLE_ID`, `ORBIT_PUSH_RELAY_DISABLED=true`.
...
1. Apple **Keys** (not Certificates) → create an **APNs Auth Key** (`.p8`) → Key ID + Team ID;
   enable **Push Notifications** on App ID `com.orbit.mobile`.
2. In the **orbit-website** repo → Actions secrets: `APNS_P8` (PEM), `APNS_KEY_ID`,
