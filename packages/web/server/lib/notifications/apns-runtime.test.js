...
const APNS_CONFIG = { keyId: 'KEY123', teamId: 'TEAM123', p8: P8, bundleId: 'com.orbit.mobile', environment: 'sandbox' };
...
afterEach(() => {
  vi.unstubAllGlobals();
  delete process.env.ORBIT_PUSH_RELAY_URL;
  delete process.env.ORBIT_PUSH_RELAY_DISABLED;
  delete process.env.ORBIT_APNS_ENVIRONMENT;
});
...
describe('apns runtime relay mode (default)', () => {
  it('registers tokens (signed) and posts signed generic text, dropping dead tokens', async () => {
    ...
    process.env.ORBIT_PUSH_RELAY_URL = 'https://relay.test/v1/push/send';
  ...
  it('honors an explicit sandbox environment override for every token', async () => {
    ...
    vi.stubGlobal('fetch', fetchMock);
    process.env.ORBIT_PUSH_RELAY_URL = 'https://relay.test/v1/push/send';
    process.env.ORBIT_APNS_ENVIRONMENT = 'sandbox';
...
describe('apns runtime direct fallback (relay disabled)', () => {
  ...
  it('sends each token to the APNs host of its registered environment', async () => {
    process.env.ORBIT_PUSH_RELAY_DISABLED = 'true';
    const { environment: _environment, ...configWithoutEnvironment } = APNS_CONFIG;
    const hosts = [];
    const http2 = {
      connect: (host) => {
