...
export const createPushRuntime = (deps) => {
  ...
  const resolveVapidSubject = async () => {
    const configured = process.env.ORBIT_VAPID_SUBJECT;
    if (typeof configured === 'string' && configured.trim().length > 0) {
      return configured.trim();
    }

    const originEnv = process.env.ORBIT_PUBLIC_ORIGIN;
    if (typeof originEnv === 'string' && originEnv.trim().length > 0) {
      const trimmed = originEnv.trim();
      if (isLoopbackHttpOrigin(trimmed)) {
        return 'mailto:orbit@localhost';
      }
      return trimmed;
    }
  ...
  const ensurePushInitialized = async () => {
    ...
    if (subject === 'mailto:orbit@localhost') {
      console.warn('[Push] No public origin configured for VAPID; set ORBIT_VAPID_SUBJECT or enable push once from a real origin.');
    }
