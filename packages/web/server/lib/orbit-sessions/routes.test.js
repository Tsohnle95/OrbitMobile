...
globalThis.__orbitCreateWorktreeMock = createWorktreeMock;
globalThis.__orbitGetWorktreeBootstrapStatusMock = getWorktreeBootstrapStatusMock;

let registerOrbitSessionRoutes;
...
vi.mock('../git/index.js', () => ({
  createWorktree: (...args) => globalThis.__orbitCreateWorktreeMock(...args),
  getWorktreeBootstrapStatus: (...args) => globalThis.__orbitGetWorktreeBootstrapStatusMock(...args),
}));
...
const createApp = (overrides = {}, options = {}) => {
  ...
  const calls = [];
  registerOrbitSessionRoutes(app, {
...
describe('orbit session routes', () => {
  beforeAll(async () => {
    ({ registerOrbitSessionRoutes } = await import('./routes.js'));
  });
  ...
  it('creates a worktree before creating a session', async () => {
    ...
    try {
      ...
      const response = await request(app)
        ...
        .send({
          directory: '/repo/app',
          worktree: { name: 'side-task', branchName: 'orbit/side-task', startRef: 'main' },
          setUpstream: false,
          prompt: 'Run this',
          model: 'openai/gpt-5.5',
        })
  ...
  it('sends a goal prompt to an existing session after creating goal metadata', async () => {
    ...
    try {
      ...
      const { app } = createApp({ createSessionGoal });
      const response = await request(app)
        .post('/api/orbit/sessions/ses_source/send')
        .send({
          directory: '/repo/app',
  ...
  it('forks from a message, dispatches the prompt, and emits the new session', async () => {
    ...
    try {
      const { app } = createApp({ emitSessionCreatedEvent });
      const response = await request(app)
        .post('/api/orbit/sessions/ses_source/fork')
        .send({
          directory: '/repo/app',
