...
import { OrbitControlError, asControlError } from '../orbit-control/error.js';
...
const waitForWorktreeBootstrapReady = async ({ directory }) => {
  ...
  for (;;) {
    ...
    if (status?.status === 'failed') {
      throw new OrbitControlError(`Worktree bootstrap failed: ${status.error || 'unknown error'}`, 500);
    }
    const phase = status?.phase;
    if (status?.status === 'ready' || phase === 'git-ready' || phase === 'setup-ready') return;
    if (Date.now() >= deadline) {
      throw new OrbitControlError('Timed out waiting for the worktree bootstrap', 500);
    }
...
export const createOrbitSessionService = (dependencies) => {
  const {
  ...
  const validateRequestedSelection = async ({ directory, requestedModel, requestedAgent, requestedVariant }) => {
    ...
    if (requestedAgent && agents.length > 0) {
      ...
      if (!agent) {
        throw new OrbitControlError(`Unknown agent '${requestedAgent}' for ${directory}`, 400);
      }
      if (!isPrimaryAgentMode(agent.mode)) {
        throw new OrbitControlError(`Agent '${requestedAgent}' is a subagent and cannot receive a prompt directly`, 400);
      }
    }

    if (requestedModel && providers.length > 0) {
      if (!hasProviderModel(providers, requestedModel.providerID, requestedModel.modelID)) {
        throw new OrbitControlError(
  ...
  const dispatchPrompt = async ({
    ...
    if (goalInput.enabled) {
      ...
      await (createSessionGoalOverride || createSessionGoal)({
        ...
        onWarning: (message, error) => console.warn(`[OrbitSessions] ${message}:`, error?.message || error),
      });
    }
  ...
  const create = async (payload = {}) => {
    ...
    if (!goalInput.ok) {
      throw new OrbitControlError(goalInput.error, 400);
    }
    ...
    if (!resolvedDirectory.ok) {
      throw new OrbitControlError(resolvedDirectory.error, resolvedDirectory.status || 400);
    }
    ...
    if (payload?.worktree && !worktreeInput) {
      throw new OrbitControlError('worktree.name is required when worktree is provided', 400);
    }
  ...
  const runExisting = async (action, sourceSessionId, payload = {}) => {
    ...
    const prompt = asNonEmptyString(payload.prompt);
    if (!sourceSessionID) throw new OrbitControlError('sessionId is required', 400);
    if (!prompt) throw new OrbitControlError('prompt is required', 400);
    const goalInput = resolveGoalInput(payload, prompt);
    if (!goalInput.ok) throw new OrbitControlError(goalInput.error, 400);
...
export const registerOrbitSessionRoutes = (app, dependencies) => {
  const service = dependencies.sessionService || createOrbitSessionService(dependencies);

  app.post('/api/orbit/sessions', express.json({ limit: '1mb' }), async (req, res) => {
    try {
      return res.json(await service.create(req.body && typeof req.body === 'object' ? req.body : {}));
    } catch (error) {
      console.error('[OrbitSessions] failed to create session:', error);
      return sendServiceError(res, error, 'Failed to create session');
    }
  });

  app.post(
    '/api/orbit/sessions/:sessionId/send',
    express.json({ limit: '1mb' }),
    async (req, res) => {
      try {
        return res.json(await service.send(req.params.sessionId, req.body));
      } catch (error) {
        console.error('[OrbitSessions] failed to send session:', error);
        return sendServiceError(res, error, 'Failed to send session');
      }
    },
  );
  app.post(
    '/api/orbit/sessions/:sessionId/fork',
    express.json({ limit: '1mb' }),
    async (req, res) => {
      try {
        return res.json(await service.fork(req.params.sessionId, req.body));
      } catch (error) {
        console.error('[OrbitSessions] failed to fork session:', error);
