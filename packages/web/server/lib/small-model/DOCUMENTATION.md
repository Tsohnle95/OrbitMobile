...
plugins — this module replicates that mechanism as an Orbit runtime API.
...
- `resolve.js` — model selection, mirroring OpenCode's `getSmallModel` chain:
  0. Orbit's own settings override (Settings → Sessions → Small Model):
