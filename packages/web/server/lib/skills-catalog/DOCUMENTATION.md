...
- Scan results persist to `skills-catalog-cache.json` in the Orbit data dir (debounced, atomic rename) and survive server restarts within the TTL.
...
- Scan and GitHub metadata caches persist to JSON files in the Orbit data dir, so app restarts and page refreshes reuse previous results instead of re-hitting GitHub.
