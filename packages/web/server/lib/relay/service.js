...
// client, and the /api/orbit/relay/* management routes.
//
...
// Routes are registered with the other Orbit feature routes, before the
...
const envRelayUrlOverride = () => {
  const raw = process.env.ORBIT_RELAY_URL;
...
export const createRelayService = ({
  ...
  const readConfig = async () => {
    ...
    return {
      ...
      // True when the endpoint is pinned by ORBIT_RELAY_URL (a self-hosted
  ...
  const standbyStatus = (holderPid) => ({
    state: 'standby',
    lastError: `relay host is owned by another local Orbit process (pid ${holderPid})`,
    connectedClients: 0,
  });
  ...
  const registerRoutes = (app) => {
    app.get('/api/orbit/relay/status', async (_req, res) => {
      try {
    ...
    app.post('/api/orbit/relay/enable', express.json({ limit: '16kb' }), async (req, res) => {
      try {
    ...
    app.post('/api/orbit/relay/disable', async (_req, res) => {
      try {
        const current = await readConfig();
