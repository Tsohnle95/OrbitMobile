 ...
 */
export const ORBIT_CONTROL_ACTION_DEFINITIONS = Object.freeze([
...
const ORBIT_CONTROL_ACTIONS = Object.freeze(
  ORBIT_CONTROL_ACTION_DEFINITIONS.map(({ action }) => action),
);

export const ORBIT_AGENT_TOOL_ACTION_DEFINITIONS = Object.freeze(
  ORBIT_CONTROL_ACTION_DEFINITIONS.filter(({ agentExposed }) => agentExposed !== false),
);

export const ORBIT_AGENT_TOOL_ACTIONS = Object.freeze(
  ORBIT_AGENT_TOOL_ACTION_DEFINITIONS.map(({ action }) => action),
);

export const ORBIT_WEB_ACTION_DEFINITIONS = Object.freeze([
...
export const ORBIT_WEB_ACTIONS = Object.freeze(
  ORBIT_WEB_ACTION_DEFINITIONS.map(({ action }) => action),
);
 ...
 */
export const ORBIT_MEMORY_ACTION_DEFINITIONS = Object.freeze([
...
export const ORBIT_MEMORY_ACTIONS = Object.freeze(
  ORBIT_MEMORY_ACTION_DEFINITIONS.map(({ action }) => action),
);
 ...
 * for `memory.read` from a tool already called `orbit_memory`, they send
...
const ACTIONS_BY_TOOL = Object.freeze({
  orbit: ORBIT_AGENT_TOOL_ACTIONS,
  orbit_web: ORBIT_WEB_ACTIONS,
  orbit_memory: ORBIT_MEMORY_ACTIONS,
});
...
export const resolveAgentToolAction = (requested, toolName) => {
  ...
  const known = scoped ?? ORBIT_ALL_ACTIONS;
  ...
  if (value) {
    const resolved = uniqueMatch(known, value)
      ...
      ?? (scoped ? null : uniqueMatch(ORBIT_ALL_ACTIONS, value));
    if (resolved) {
  ...
  return {
    error: `Unsupported Orbit action: ${value || 'missing'}. Use one of: ${known.join(', ')}`,
  };
};

/** Everything the callback route will dispatch, whichever tool asked. */
export const ORBIT_ALL_ACTIONS = Object.freeze([
  ...ORBIT_CONTROL_ACTIONS,
  ...ORBIT_WEB_ACTIONS,
