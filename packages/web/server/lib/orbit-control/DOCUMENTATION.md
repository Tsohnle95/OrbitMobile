# Orbit Control Service

## Purpose

This module owns the typed control contract shared by the Orbit CLI and
the managed OpenCode `orbit` tool. Both adapters delegate to
`createOrbitControlService()`; neither adapter may call or spawn the
other.
...
- `service.js` validates and executes the fixed project, model, session, and
  ...
  the filtered `ORBIT_AGENT_TOOL_*` exports. `schedule.toggle` requires
...
- `../agent-tool/runtime.js` is the managed-tool adapter. It wraps service
  ...
  loopback credential.
- `../orbit-sessions/routes.js` and `../scheduled-tasks/service.js` own
...
- `browser.capture` writes its image on the server, into
  `.orbit/screenshots/` under the scoped project directory, and returns
