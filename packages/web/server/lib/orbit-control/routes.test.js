...
import { OrbitControlError } from './error.js';
import { registerOrbitControlRoutes } from './routes.js';

const createApp = (execute) => {
  const app = express();
  registerOrbitControlRoutes(app, { controlService: { execute } });
  return app;
};

describe('Orbit control route', () => {
  ...
  it('preserves service status and partial-result details', async () => {
    const execute = vi.fn(async () => {
      throw new OrbitControlError('dispatch failed', 500, {
        partial: true,
