...
import { OrbitControlError, asControlError } from './error.js';
import { ORBIT_ALL_ACTIONS } from './actions.js';
...
const CONTROL_ACTIONS = new Set(ORBIT_ALL_ACTIONS);
...
const positiveInteger = (value, fallback, field) => {
  ...
  if (!Number.isSafeInteger(number) || number < 1) {
    throw new OrbitControlError(`${field} must be a positive integer`, 400);
  }
  return number;
};
...
const normalizeWaitTimeoutMs = (value) => {
  ...
  if (!Number.isSafeInteger(seconds) || seconds < 1 || seconds > MAX_WAIT_TIMEOUT_SECONDS) {
    throw new OrbitControlError(`timeout must be from 1 to ${MAX_WAIT_TIMEOUT_SECONDS} seconds`, 400);
  }
  return seconds * 1000;
};
...
const parseModel = (value) => {
  ...
  if (!model) throw new OrbitControlError('model is required', 400);
  const slashIndex = model.indexOf('/');
  if (slashIndex <= 0 || slashIndex === model.length - 1) {
    throw new OrbitControlError('model must be in provider/model format', 400);
  }
...
const parseWeekdays = (value) => {
  ...
  if (!raw) throw new OrbitControlError('weekly is required', 400);
  ...
  if (weekdays.some((entry) => !Number.isInteger(entry) || entry < 0 || entry > 6)) {
    throw new OrbitControlError('weekly must contain weekdays from 0 to 6', 400);
  }
...
const buildSchedule = (input) => {
  ...
  if (selectors.length !== 1) {
    throw new OrbitControlError('Provide exactly one of daily, weekly, once, or cron', 400);
  }
  ...
  if (weekly) {
    ...
    if (!time) throw new OrbitControlError('time is required with weekly', 400);
    return { kind: 'weekly', weekdays: parseWeekdays(weekly), times: [time], ...(timezone ? { timezone } : {}) };
  }
  if (once) {
    const time = asNonEmptyString(input.time);
    if (!time) throw new OrbitControlError('time is required with once', 400);
...
const buildScheduledTask = (input) => {
  ...
  if (!name) throw new OrbitControlError('name is required', 400);
  if (!prompt) throw new OrbitControlError('prompt is required', 400);
  const model = parseModel(input.model);
  const goalTokenBudget = input.goalTokenBudget;
  if (goalTokenBudget !== undefined && input.goal !== true) {
    throw new OrbitControlError('goalTokenBudget requires goal', 400);
  }
  if (goalTokenBudget !== undefined && (!Number.isSafeInteger(goalTokenBudget) || goalTokenBudget < 1000 || goalTokenBudget > 100_000_000)) {
    throw new OrbitControlError('goalTokenBudget must be from 1000 to 100000000', 400);
  }
  return {
    name,
...
export const createOrbitControlService = (dependencies) => {
  const {
  ...
  const wait = (duration, signal) => {
    ...
    if (signal.aborted) return Promise.reject(new OrbitControlError('Orbit action was cancelled', 499));
    return new Promise((resolve, reject) => {
      const onAbort = () => {
        signal.removeEventListener('abort', onAbort);
        reject(new OrbitControlError('Orbit action was cancelled', 499));
      };
  ...
  const sessionStatus = async (client, sessionID, directory) => {
    ...
    if (!statuses || typeof statuses !== 'object' || Array.isArray(statuses)) {
      throw new OrbitControlError('Invalid session status response', 500);
    }
  ...
  const waitForIdle = async ({ client, sessionID, directory, timeoutMs, requireActivity, baselineMessageID, startedAt, signal }) => {
    ...
    while (true) {
      if (signal?.aborted) throw new OrbitControlError('Orbit action was cancelled', 499);
      ...
      if (remaining <= 0) {
        throw new OrbitControlError(`Session did not become idle within ${Math.ceil(timeoutMs / 1000)} seconds`, 500);
      }
  ...
  const executeSessionAction = async (action, input, contextDirectory, signal) => {
    if (input.timeout !== undefined && input.wait !== true) throw new OrbitControlError('timeout requires wait', 400);
    if (input.lastAssistant === true && input.wait !== true) throw new OrbitControlError('lastAssistant requires wait', 400);
    ...
    if (action === 'session.create') {
      ...
      if (!sessionID) throw new OrbitControlError('sessionId is required', 400);
  ...
  const browserAction = async (action, input, signal, contextDirectory) => {
    ...
    const readViewport = (required) => {
      ...
      if (!viewport) {
        if (required) throw new OrbitControlError('viewport is required for browser.resize', 400);
        return;
      }
      if (!['mobile', 'tablet', 'desktop', 'fill'].includes(viewport)) {
        throw new OrbitControlError('viewport must be mobile, tablet, desktop, or fill', 400);
      }
    ...
    if (action === 'browser.open') {
      ...
      if (!url) throw new OrbitControlError('url is required for browser.open', 400);
      let parsed;
      try {
        parsed = new URL(url);
      } catch {
        throw new OrbitControlError('url must be an absolute http(s) URL', 400);
      }
      if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
        throw new OrbitControlError('url must use http or https', 400);
      }
    ...
    if (action === 'browser.click') {
      ...
      if (!selector && !text) {
        throw new OrbitControlError('browser.click requires selector or text', 400);
      }
    ...
    if (action === 'browser.inspect') {
      const selector = asNonEmptyString(input.selector);
      if (!selector) throw new OrbitControlError('selector is required for browser.inspect', 400);
      parameters.selector = selector;
    }

    if (action === 'browser.type') {
      const selector = asNonEmptyString(input.selector);
      if (!selector) throw new OrbitControlError('selector is required for browser.type', 400);
      if (typeof input.value !== 'string') {
        throw new OrbitControlError('value is required for browser.type', 400);
      }
      parameters.selector = selector;
    ...
    if (action === 'browser.scroll') {
      ...
      if (!selector && !direction) {
        throw new OrbitControlError('browser.scroll requires direction or selector', 400);
      }
      if (direction && !['up', 'down', 'top', 'bottom'].includes(direction)) {
        throw new OrbitControlError('direction must be up, down, top, or bottom', 400);
      }
      if (selector) parameters.selector = selector;
    ...
    if (action === 'browser.capture') {
      ...
      if (!directory) {
        throw new OrbitControlError('directory is required to save a screenshot', 400);
      }
  ...
  const execute = async (action, input = {}, contextDirectory, options = {}) => {
    try {
      if (!CONTROL_ACTIONS.has(action)) {
        throw new OrbitControlError(`Unsupported Orbit action: ${action || 'missing'}`, 400);
      }
      if (action.startsWith('memory.')) {
        if (!agentMemoryActions) {
          throw new OrbitControlError('Agent memory is not available on this server', 503);
        }
        return agentMemoryActions.execute(action, input, contextDirectory);
      }
      if (action.startsWith('browser.')) {
        if (!browserControl) {
          throw new OrbitControlError('The in-app browser is not available on this server', 503);
        }
      ...
      if (action.startsWith('schedule.')) {
        ...
        if (SCHEDULE_TASK_ID_ACTIONS.has(action) && !taskID) {
          throw new OrbitControlError('taskId is required', 400);
        }
        ...
        switch (action) {
          ...
          case 'schedule.toggle': {
            if (typeof input.disabled !== 'boolean') {
              throw new OrbitControlError('disabled is required for schedule.toggle', 400);
            }
      ...
      if (action.startsWith('session.')) {
        ...
        if (!sessionID) throw new OrbitControlError('sessionId is required', 400);
        if (!directory) throw new OrbitControlError('directory is required', 400);
        ...
        if (action === 'session.messages') {
          ...
          if (!['all', 'user', 'assistant'].includes(role)) throw new OrbitControlError('role must be all, user, or assistant', 400);
          const last = input.last === true || input.lastAssistant === true;
          if (input.all === true && (last || input.limit !== undefined)) throw new OrbitControlError('all cannot be combined with last or limit', 400);
          if (last && input.limit !== undefined) throw new OrbitControlError('last cannot be combined with limit', 400);
      ...
      throw asControlError(error, `Failed to execute ${action || 'Orbit action'}`);
    }
  };
