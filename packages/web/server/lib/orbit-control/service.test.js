...
import { createOrbitControlService } from './service.js';
...
const createService = (overrides = {}) => {
  ...
  const service = createOrbitControlService({
...
describe('Orbit control service', () => {
  ...
  it('rejects actions outside the fixed contract', async () => {
    const { service } = createService();
    await expect(service.execute('session.delete')).rejects.toThrow('Unsupported Orbit action');
  });
});
...
describe('browser capture', () => {
  ...
  it('saves the image beside the code and hands back a path the answer can use', async () => {
    ...
    expect(result.path.startsWith('.orbit/screenshots/after-fix-')).toBe(true);
