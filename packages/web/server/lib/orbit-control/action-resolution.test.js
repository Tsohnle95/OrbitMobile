 ...
 * then `get` on `orbit_memory`, having dropped the namespace its own tool
...
describe('a namespace the tool name already implies', () => {
  test('resolves a bare action inside the calling tool', () => {
    expect(resolveAgentToolAction('read', 'orbit_memory')).toEqual({ action: 'memory.read' });
    expect(resolveAgentToolAction('save', 'orbit_memory')).toEqual({ action: 'memory.save' });
  });
  ...
  test('resolves a bare name that is ambiguous only across tools', () => {
    ...
    expect(resolveAgentToolAction('delete', 'orbit_memory')).toEqual({ action: 'memory.delete' });
    expect(resolveAgentToolAction('delete', 'orbit')).toEqual({ action: 'schedule.delete' });
  });

  test('keeps a fully qualified action as it is', () => {
    expect(resolveAgentToolAction('memory.read', 'orbit_memory')).toEqual({ action: 'memory.read' });
  });

  test('does not reach outside the tool that asked', () => {
    // The memory tool asking for `open` must fail, not drive the browser.
    expect(resolveAgentToolAction('open', 'orbit_memory').action).toBeUndefined();
  });
});
...
describe('what an unresolvable action reports', () => {
  test('names the actions the calling tool actually has', () => {
    const { error } = resolveAgentToolAction('get', 'orbit_memory');
  ...
  test('reports a missing action rather than resolving to something', () => {
    const { error, action } = resolveAgentToolAction('', 'orbit_memory');
  ...
  test('an unknown tool falls back to the full action list', () => {
    const { error } = resolveAgentToolAction('nonsense', 'orbit_future');
