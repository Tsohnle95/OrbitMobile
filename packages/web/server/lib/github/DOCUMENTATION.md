...
- This module owns GitHub auth, Octokit access, repo resolution, and Pull Request status resolution for Orbit.
...
- Auth storage: `~/.config/orbit/github-auth.json`
- Writes are atomic and file mode is `0o600`.
- Client ID resolution order: `ORBIT_GITHUB_CLIENT_ID` -> `settings.json` -> default.
- Scope resolution order: `ORBIT_GITHUB_SCOPES` -> `settings.json` -> default.
...
- PR state is persisted in local storage under `orbit.github-pr-status`.
