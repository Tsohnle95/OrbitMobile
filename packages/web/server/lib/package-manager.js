...
const PACKAGE_NAME = '@orbit/web';
const PACKAGE_PATH_SEGMENTS = PACKAGE_NAME.split('/');
const NPM_REGISTRY_URL = `https://registry.npmjs.org/${PACKAGE_NAME}`;
const CHANGELOG_URL = 'https://raw.githubusercontent.com/openchamber/orbit/main/CHANGELOG.md';
const GITHUB_RELEASES_URL = 'https://github.com/openchamber/orbit/releases';
const GITHUB_RELEASES_API_URL = 'https://api.github.com/repos/orbit/orbit/releases';
let cachedDetectedPm = null;

function getSpawnSyncBaseOptions() {
  return process.platform === 'win32' ? { windowsHide: true } : {};
}
const UPDATE_CHECK_URL = process.env.ORBIT_UPDATE_API_URL || 'https://api.openchamber.dev/v1/update/check';

function getOrbitConfigDir() {
  if (process.platform === 'win32') {
    const appData = process.env.APPDATA;
    if (appData) return path.join(appData, 'orbit');
  }
...
function getOrCreateInstallId(scope = 'web') {
  const configDir = getOrbitConfigDir();
...
async function resolveAndroidApkUrl(version, candidateUrl) {
  ...
  try {
    ...
    const apkAssets = Array.isArray(release?.assets)
      ...
      : [];
    const canonicalAsset = apkAssets.find((asset) => /^Orbit-.+-android\.apk$/i.test(asset.name));
...
function getOwnedPackagePathsFromGlobalBins(pm) {
  ...
  for (const binDir of getGlobalBinDirs(pm)) {
    const binaryName = process.platform === 'win32' ? 'orbit.cmd' : 'orbit';
...
export function detectPackageManagerDetails() {
  ...
  const forcedPm = process.env.ORBIT_PACKAGE_MANAGER?.trim();
...
function isPackageInstalledWith(pm) {
  try {
    ...
    return result.stdout.includes(PACKAGE_NAME) || result.stdout.includes('orbit');
  } catch {
    return false;
  }
}
