...
describe('normalizeCustomOpenAIBaseURL', () => {
  const originalRuntime = process.env.ORBIT_RUNTIME;
  const originalAllowRemote = process.env.ORBIT_ALLOW_REMOTE_OPENAI_COMPAT_URLS;

  afterEach(() => {
    // Restore env vars after each test
    if (originalRuntime === undefined) {
      delete process.env.ORBIT_RUNTIME;
    } else {
      process.env.ORBIT_RUNTIME = originalRuntime;
    }
    if (originalAllowRemote === undefined) {
      delete process.env.ORBIT_ALLOW_REMOTE_OPENAI_COMPAT_URLS;
    } else {
      process.env.ORBIT_ALLOW_REMOTE_OPENAI_COMPAT_URLS = originalAllowRemote;
    }
  });

  it('rejects remote URLs when ORBIT_RUNTIME is not set (web)', () => {
    delete process.env.ORBIT_RUNTIME;
    delete process.env.ORBIT_ALLOW_REMOTE_OPENAI_COMPAT_URLS;
  ...
  it('allows remote URLs when ORBIT_RUNTIME is desktop', () => {
  ...
  it('allows remote URLs when ORBIT_ALLOW_REMOTE_OPENAI_COMPAT_URLS is true', () => {
    delete process.env.ORBIT_RUNTIME;
    process.env.ORBIT_ALLOW_REMOTE_OPENAI_COMPAT_URLS = 'true';
  ...
  it('denies remote URLs on desktop when env var is explicitly false', () => {
    process.env.ORBIT_RUNTIME = 'desktop';
    process.env.ORBIT_ALLOW_REMOTE_OPENAI_COMPAT_URLS = 'false';
