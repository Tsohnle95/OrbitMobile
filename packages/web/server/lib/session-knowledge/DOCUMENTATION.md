...
`session.metadata.orbit.project_context_pins` owns the note and plan ids
...
Directories beneath the managed `~/.config/orbit/chats` root resolve to that root before project context and project memory are read. Every ordinary chat therefore shares one Chats knowledge owner instead of creating an unreachable context store for each dated session directory.

`session.metadata.orbit.knowledge_context_delivered` holds the signature
...
Agent memory is complete but unreleased. `ORBIT_MEMORY_ENABLE` decides
