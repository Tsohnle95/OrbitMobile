...
const buildMemoryBlock = ({ global, project }) => {
  ...
  return [
    ...
    'A title is an abbreviation, not the memory. Read the entry with the'
      + ' orbit_memory tool before you act on it: titles routinely leave out'
...
export const createSessionKnowledgeRuntime = (dependencies) => {
  ...
  const readPins = (session) => {
    ...
    const pins = isRecord(orbit[PINS_METADATA_KEY]) ? orbit[PINS_METADATA_KEY] : {};
  ...
  const readDeliveredSignature = (session) => {
    const metadata = isRecord(session?.metadata) ? session.metadata : {};
    const orbit = isRecord(metadata.orbit) ? metadata.orbit : {};
    const delivered = orbit[KNOWLEDGE_METADATA_KEY];
   ...
   * Orbit state — pinned messages among it — and a blind write would
  ...
  const recordDelivered = async (sessionId, directory, signature) => {
    ...
    await openCodeFetch(`/session/${encodeURIComponent(sessionId)}`, {
      ...
      body: {
        metadata: {
          ...metadata,
          orbit: { ...orbit, [KNOWLEDGE_METADATA_KEY]: signature },
        },
      },
    });
  };
