...
const readContextState = (session) => {
  ...
  const messages = Array.isArray(orbit.context_obligatory_messages)
    ? orbit.context_obligatory_messages.filter((item) =>
      isRecord(item)
      && typeof item.id === 'string'
      && typeof item.createdAt === 'number'
      && (item.role === 'user' || item.role === 'assistant'))
    : [];
  return { metadata, orbit, messages };
};
...
export const createContextObligatoryRuntime = ({
  ...
  const tick = async (sessionId, directory) => {
    ...
    if (state.orbit.context_obligatory_last_compaction_message_id === summary.id) return;
