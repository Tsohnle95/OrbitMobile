...
The managed Chats root (`~/.config/orbit/chats`) is also one context owner. Every dated per-session directory beneath it resolves to that root, so Notes, Todo, Plans, pinned knowledge, and project memory are shared across ordinary chats without registering Chats as a user project.

## Ownership

| Path | Owner | Contents |
|---|---|---|
| `<projectsDir>/<projectId>.json` | shared UI (`packages/ui/src/lib/orbitConfig.ts`), plus server-owned `version` / `scheduledTasks` | worktree setup, draft starters, project actions |
