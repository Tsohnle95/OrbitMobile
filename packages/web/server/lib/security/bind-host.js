...
export const isUnsafeUnauthenticatedLanAllowed = (env = process.env) =>
  env?.ORBIT_ALLOW_UNAUTHENTICATED_LAN === 'true';

export const getUnauthenticatedLanErrorMessage = (host) =>
  `Orbit refuses to bind to ${host || 'a network-exposed host'} without UI authentication. `
  + 'Set --ui-password or ORBIT_UI_PASSWORD before exposing it over LAN, '
  + 'or set ORBIT_ALLOW_UNAUTHENTICATED_LAN=true to accept the risk.';
