...
const RATE_LIMIT_MAX_ATTEMPTS = Number(process.env.ORBIT_RATE_LIMIT_MAX_ATTEMPTS) || 10;
const RATE_LIMIT_LOCKOUT_MS = 15 * 60 * 1000;
const RATE_LIMIT_CLEANUP_MS = 60 * 60 * 1000;
const RATE_LIMIT_NO_IP_MAX_ATTEMPTS = Number(process.env.ORBIT_RATE_LIMIT_NO_IP_MAX_ATTEMPTS) || 3;
...
const isUrlAuthReadableHttpPath = (pathname) => {
  return pathname === '/api/event'
    ...
    || pathname === '/api/orbit/events'
    || pathname === '/api/orbit/realtime-proxy/sse'
...
const isUrlAuthWebSocketPath = (pathname) => {
  return pathname === '/api/event/ws'
    ...
    || pathname === '/api/orbit/realtime-proxy/ws'
...
const JWT_SECRET_FILE = path.join(ORBIT_DATA_DIR, 'jwt-secret');
...
function getOrCreateJwtSecret() {
  ...
  try {
    fs.mkdirSync(ORBIT_DATA_DIR, { recursive: true });
