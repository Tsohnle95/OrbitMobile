...
## Purpose
This module contains the Orbit message-stream WebSocket protocol and runtime bridge. It keeps the browser-facing WebSocket transport separate from the upstream OpenCode SSE transport.
...
- Orbit still fetches OpenCode upstream event streams over SSE.
...
- Global synthetic events such as `orbit:session-status`, `orbit:session-activity`, `orbit:notification`, and `orbit:heartbeat` are preserved on the WS path, but heartbeat frames are emitted only while an upstream SSE stream is actively attached.
