...
objective stored under `metadata.orbit.goal`, with the small model as
an independent progress auditor. Built on Orbit's backend-driven
architecture (session-assist is the structural template): the loop lives in
the web server and survives UI disconnects.

## Goal payload (`metadata.orbit.goal`)
...
`ORBIT_DATA_DIR` or `~/.config/orbit`), keyed by the SESSION ID:
...
- `routes.js` — `PUT/GET/DELETE /api/goals/objective/:sessionId`
  (Orbit-owned, registered before the generic proxy; JSON parsing via
...
stamp `metadata.orbit.goal` onto the fresh session (objective = the
...
`orbit session create --prompt <text> --goal` uses the explicit
`POST /api/orbit/sessions` orchestration route. The server creates the
...
`orbit session send --goal` and `orbit session fork --goal` use
