...
// (metadata.orbit.goal). While the goal is active, the server keeps the
...
const ORBIT_SETTINGS_FILE = path.join(
...
const isSessionGoalEnabled = () => {
  try {
    const raw = fs.readFileSync(ORBIT_SETTINGS_FILE, 'utf8');
...
const parseGoalMetadata = (session) => {
  ...
  const namespace = metadata.orbit;
...
export const createSessionGoalRuntime = ({
  ...
  const writeGoal = async (sessionId, directory, expectedGoalId, mutate) => {
    ...
    const currentNamespace = currentMetadata.orbit && typeof currentMetadata.orbit === 'object'
      ? currentMetadata.orbit
      : {};
    await openCodeFetch(`/session/${encodeURIComponent(sessionId)}`, {
      ...
      body: {
        metadata: {
          ...currentMetadata,
          orbit: { ...currentNamespace, goal: nextGoal },
        },
      },
    });
    return nextGoal;
  ...
  const tick = async (sessionId, directory) => {
    ...
    // TEXT lives under the Orbit data dir keyed by session id and is
