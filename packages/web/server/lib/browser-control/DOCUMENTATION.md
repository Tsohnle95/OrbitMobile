...
- `../../index.js` supplies `emitRequest`, which writes the request to the
  Orbit SSE clients and returns how many were reached.
- `../orbit-control/service.js` is the only caller. It maps the
  `browser.*` actions of the `orbit_web` tool onto `broker.request()` and
