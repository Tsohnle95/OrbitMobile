...
import { ORBIT_AGENT_TOOL_ACTION_DEFINITIONS, ORBIT_CONTROL_ACTION_DEFINITIONS } from '../orbit-control/actions.js';
...
const createRuntime = async (overrides = {}) => {
  const dataDir = await fs.mkdtemp(path.join(os.tmpdir(), 'orbit-agent-tool-'));
...
describe('agent tool action allowlist', () => {
  it('defines a short title and agent description for every action', () => {
    expect(ORBIT_CONTROL_ACTION_DEFINITIONS.every(({ action, title, description }) => action && title && description)).toBe(true);
  });
...
describe('managed agent tool runtime', () => {
  it('materializes the plugin and preserves configured plugin entries', async () => {
    ...
    const pluginPath = path.join(dataDir, 'agent-tool', 'orbit-plugin.js');
    ...
    expect(config.plugin).toEqual([
      ...
      expect.stringContaining('/agent-tool/orbit-plugin.js'),
    ]);
    expect(preparedEnv.ORBIT_AGENT_TOOL_URL).toBe('http://127.0.0.1:3901/api/orbit/agent-tool');
    expect(preparedEnv.ORBIT_AGENT_TOOL_TOKEN).toMatch(/^[A-Za-z0-9_-]+$/);
    expect(source).toContain('orbit: {');
    for (const { action, description } of ORBIT_AGENT_TOOL_ACTION_DEFINITIONS) {
    ...
    const hooks = await pluginModule.OrbitPlugin();
    expect(hooks.tool.orbit.description).toContain('Session dispatches return immediately by default');
    expect(hooks.tool.orbit.description).toContain('Set wait only when the user asks or the next step requires the completed result');
    expect(hooks.tool.orbit.args.action.oneOf).toContainEqual({
      const: 'session.messages',
      description: 'Read text-only messages and current sessionStatus for sessionId; directory and limit 10 are defaults',
    });
    expect(hooks.tool.orbit.args.parameters.properties.wait.description).toBe(
    ...
    expect(hooks.tool.orbit.args.parameters.properties.sessionId).toEqual({ type: 'string' });
    expect(source).not.toContain('title: "Orbit"');
    expect(source).not.toContain('@opencode-ai/plugin');
    expect(source).not.toContain(preparedEnv.ORBIT_AGENT_TOOL_TOKEN);
  });
  ...
  it('emits both tools, each carrying only its own actions and inputs', async () => {
    ...
    const { tool } = await pluginModule.OrbitPlugin();

    const controlActions = tool.orbit.args.action.enum;
    const webActions = tool.orbit_web.args.action.enum;
    ...
    expect(Object.keys(tool.orbit_web.args.parameters.properties)).toContain('url');
    expect(Object.keys(tool.orbit.args.parameters.properties)).not.toContain('url');
    expect(Object.keys(tool.orbit.args.parameters.properties)).toContain('sessionId');
  });
  ...
  it('accepts inputs passed beside the action, not only inside parameters', async () => {
    ...
    const originalUrl = process.env.ORBIT_AGENT_TOOL_URL;
    const originalToken = process.env.ORBIT_AGENT_TOOL_TOKEN;
    process.env.ORBIT_AGENT_TOOL_URL = prepared.ORBIT_AGENT_TOOL_URL;
    process.env.ORBIT_AGENT_TOOL_TOKEN = prepared.ORBIT_AGENT_TOOL_TOKEN;
    ...
    try {
      ...
      await tool.orbit.execute(
        { action: 'session.messages', sessionId: 'ses_1', limit: 3 },
        context,
      );
    } finally {
      globalThis.fetch = originalFetch;
      process.env.ORBIT_AGENT_TOOL_URL = originalUrl;
      process.env.ORBIT_AGENT_TOOL_TOKEN = originalToken;
    }
  ...
  it('omits a tool the user turned off', async () => {
    ...
    expect(Object.keys(tool)).toEqual(['orbit_web']);
  });
  ...
  it('exposes memory as its own tool carrying only its own inputs', async () => {
    ...
    expect(Object.keys(tool)).toEqual(['orbit', 'orbit_memory']);
    expect(Object.keys(tool.orbit_memory.args.parameters.properties).sort())
    ...
    expect(Object.keys(tool.orbit.args.parameters.properties)).not.toContain('memoryId');
  });
  ...
  it('omits memory entirely when the user turns it off', async () => {
    ...
    expect(Object.keys(tool)).toEqual(['orbit']);
  });
  ...
  it('injects the plugin when memory is the only tool left on', async () => {
    ...
    expect(Object.keys(tool)).toEqual(['orbit_memory']);
  });
  ...
  it('accepts the bare action a tool name already qualifies', async () => {
    // Observed: the model called `read` on orbit_memory, having taken the
  ...
  it('forwards cancellation to the shared control service', async () => {
    const executeAction = vi.fn(async (_action, _input, _directory, options) => {
      await new Promise((resolve, reject) => {
        options.signal.addEventListener('abort', () => reject(Object.assign(new Error('Orbit action was cancelled'), { statusCode: 499 })), { once: true });
      });
    });
    const { runtime } = await createRuntime({ executeAction });
    ...
    await expect(pending).resolves.toEqual(expect.objectContaining({
      ok: false,
      action: 'projects.list',
      error: { message: 'Orbit action was cancelled', kind: 'runtime' },
    }));
  ...
  it('requires the per-child token on the loopback route', async () => {
    ...
    await request(app)
      .post('/api/orbit/agent-tool')
      .send({ input: { action: 'projects.list' } })
      .expect(401);

    const response = await request(app)
      .post('/api/orbit/agent-tool')
      .set('authorization', `Bearer ${env.ORBIT_AGENT_TOOL_TOKEN}`)
      .send({ input: { action: 'projects.list' } })
      .expect(200);
  ...
  it('executes through the materialized plugin and authenticated callback', async () => {
    ...
    const previousUrl = process.env.ORBIT_AGENT_TOOL_URL;
    const previousToken = process.env.ORBIT_AGENT_TOOL_TOKEN;
    try {
      const env = await runtime.prepareManagedOpenCodeEnv();
      process.env.ORBIT_AGENT_TOOL_URL = env.ORBIT_AGENT_TOOL_URL;
      process.env.ORBIT_AGENT_TOOL_TOKEN = env.ORBIT_AGENT_TOOL_TOKEN;
      const pluginPath = path.join(dataDir, 'agent-tool', 'orbit-plugin.js');
      const pluginModule = await import(`${pathToFileURL(pluginPath).href}?test=${Date.now()}`);
      const hooks = await pluginModule.OrbitPlugin();
      const metadata = vi.fn();

      const result = await hooks.tool.orbit.execute(
      ...
      expect(result.metadata.orbit.description).toBe('List configured projects');
      expect(metadata).toHaveBeenCalledWith(expect.objectContaining({
        title: 'List configured projects',
        metadata: expect.objectContaining({
          orbit: expect.objectContaining({ description: 'List configured projects' }),
        }),
      }));
    } finally {
      if (previousUrl === undefined) delete process.env.ORBIT_AGENT_TOOL_URL;
      else process.env.ORBIT_AGENT_TOOL_URL = previousUrl;
      if (previousToken === undefined) delete process.env.ORBIT_AGENT_TOOL_TOKEN;
      else process.env.ORBIT_AGENT_TOOL_TOKEN = previousToken;
