# Managed Orbit Agent Tool

## Purpose

This module exposes Orbit to agents as typed OpenCode custom tools. There
are two, because controlling sessions and driving a page are separate intents
the user can want independently:

- `orbit` — projects, sessions, worktrees, and scheduled tasks. Enabled
  while the persisted `agentControlToolEnabled` setting is not `false`.
- `orbit_web` — looking at and interacting with the page in Orbit's
...
when Orbit launches and owns the OpenCode process, and not at all when
...
1. The Orbit HTTP listener binds and publishes its authoritative port.
2. `prepareManagedOpenCodeEnv()` materializes the plugin under
   `<orbit-data-dir>/agent-tool/` and appends its `file://` URL to
...
4. The plugin calls `POST /api/orbit/agent-tool` with its typed input and
   OpenCode's authoritative session directory.
5. The route delegates the fixed action allowlist directly to the shared
   Orbit control service. The CLI uses the same service through its
...
- The tool exposes only agent-relevant actions
  (`ORBIT_AGENT_TOOL_ACTIONS`): `schedule.status` stays CLI-only because
...
- External OpenCode selected with `OPENCODE_HOST` or skip-start: not injected,
  because Orbit does not control that process environment.
...
drop the namespace their tool's name appears to supply — `orbit_memory`
