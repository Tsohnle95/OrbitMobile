...
import {
  ORBIT_AGENT_TOOL_ACTION_DEFINITIONS,
  ORBIT_AGENT_TOOL_ACTIONS,
  ORBIT_MEMORY_ACTION_DEFINITIONS,
  ORBIT_MEMORY_ACTIONS,
  resolveAgentToolAction,
  ORBIT_WEB_ACTION_DEFINITIONS,
  ORBIT_WEB_ACTIONS,
} from '../orbit-control/actions.js';
...
const ACTIONS = new Set([...ORBIT_AGENT_TOOL_ACTIONS, ...ORBIT_WEB_ACTIONS, ...ORBIT_MEMORY_ACTIONS]);
...
const createToolEntry = ({ name, description, actions, definitions, parameters }) => String.raw`    ${name}: {
      ...
      args: {
        action: { type: "string", enum: ${JSON.stringify(actions)}, oneOf: ${JSON.stringify(definitions.map((entry) => ({ const: entry.action, description: entry.description })))}, description: "Orbit action to perform" },
      ...
      async execute(input, context) {
        ...
        const endpoint = process.env.ORBIT_AGENT_TOOL_URL
        const token = process.env.ORBIT_AGENT_TOOL_TOKEN
        const failure = (payload) => ({
          title,
          output: JSON.stringify(payload),
          metadata: { orbit: { schemaVersion: ${TOOL_SCHEMA_VERSION}, action: args.action, description: title, ok: false } },
        })
        if (!endpoint || !token) {
          return failure({ schemaVersion: ${TOOL_SCHEMA_VERSION}, ok: false, action: args.action, error: { message: "Orbit managed tool connection is unavailable" } })
        }
        ...
        try {
          ...
          if (valid) return { title, output, metadata: { orbit: { schemaVersion: ${TOOL_SCHEMA_VERSION}, action: args.action, description: title, ok: result.ok === true } } }
          return failure({ schemaVersion: ${TOOL_SCHEMA_VERSION}, ok: false, action: args.action, error: { message: "Orbit returned an invalid response", kind: "runtime", status: response.status } })
        } catch (error) {
...
const createPluginSource = ({ includeControl, includeWeb, includeMemory }) => {
  ...
  if (includeControl) {
    entries.push(createToolEntry({
      ...
      actions: ORBIT_AGENT_TOOL_ACTIONS,
      definitions: ORBIT_AGENT_TOOL_ACTION_DEFINITIONS,
  ...
  if (includeWeb) {
    entries.push(createToolEntry({
      ...
      actions: ORBIT_WEB_ACTIONS,
      definitions: ORBIT_WEB_ACTION_DEFINITIONS,
  ...
  if (includeMemory) {
    entries.push(createToolEntry({
      ...
      actions: ORBIT_MEMORY_ACTIONS,
      definitions: ORBIT_MEMORY_ACTION_DEFINITIONS,
      parameters: MEMORY_PARAMETER_PROPERTIES,
    }));
  }

  return `export const OrbitPlugin = async () => ({
  tool: {
${entries.join('')}  },
})
`;
...
const mergePluginConfig = (rawConfig, pluginUrl) => {
  ...
  if (errors.length > 0 || !parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    throw new Error('OPENCODE_CONFIG_CONTENT must contain a valid JSON object before Orbit can inject its managed tool');
  }
  if (parsed.plugin !== undefined && !Array.isArray(parsed.plugin)) {
    throw new Error('OPENCODE_CONFIG_CONTENT plugin must be an array before Orbit can inject its managed tool');
  }
...
export const createAgentToolRuntime = (dependencies) => {
  ...
  const pluginPath = path.join(pluginDirectory, 'orbit-plugin.js');
  let activeToken = null;

  const prepareManagedOpenCodeEnv = async ({ includeControl = true, includeWeb = true, includeMemory = true } = {}) => {
    const port = getActivePort();
    if (!Number.isInteger(port) || port <= 0) {
      throw new Error('Orbit listener port is unavailable for managed tool injection');
    }
    if (!includeControl && !includeWeb && !includeMemory) {
      throw new Error('At least one Orbit managed tool must be enabled to inject the plugin');
    }
    ...
    return {
      ...
      ORBIT_AGENT_TOOL_URL: `http://127.0.0.1:${port}/api/orbit/agent-tool`,
      ORBIT_AGENT_TOOL_TOKEN: activeToken,
    };
  };
  ...
  const execute = async (payload = {}, options = {}) => {
    ...
    if (!ACTIONS.has(action)) {
      return createResult({ ok: false, action, error: { message: `Unsupported Orbit action: ${action}`, kind: 'usage' } });
    }
    if (typeof executeAction !== 'function') {
      return createResult({ ok: false, action, error: { message: 'Orbit control service is unavailable', kind: 'runtime' } });
    }
    try {
  ...
  const registerRoutes = (app, express) => {
    app.post('/api/orbit/agent-tool', express.json({ limit: '1mb' }), async (req, res) => {
