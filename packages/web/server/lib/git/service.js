...
const createGit = async (directory, { allowUnsafeSshCommand = false } = {}) => {
  ...
  // makes simple-git use process.cwd(), which breaks when the Orbit
...
// Orbit places managed worktrees under a deep data-dir path
...
const formatWorktreePopulateError = (message) => {
  ...
  return [
    ...
    'Orbit enables Git `core.longpaths` for worktree population; if this still fails on Windows, enable OS long paths (LongPathsEnabled) or open the repository from a shorter absolute path.',
  ].join('\n');
};
...
export const ensureWorktreeLongpaths = async (directory) => {
  ...
  // subsequent Orbit and CLI git operations in this repo also get long
...
const createIntegrateTempWorktree = async (repoRoot, targetBranch) => {
  const tmpParent = path.join(os.homedir(), '.config', 'orbit', 'tmp');
...
// whose directory no longer exists when reading them back. Orbit used to
...
const writeTempPatchFile = async (patch) => {
  ...
  const tmpPath = path.join(tmpDir, `orbit-hunk-${Date.now()}-${Math.random().toString(36).slice(2)}.patch`);
...
export async function stashPush(directory, options = {}) {
  ...
  const message = typeof options.message === 'string' && options.message.trim()
    ...
    : `Orbit stash ${new Date().toISOString()}`;
