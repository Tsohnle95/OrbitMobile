...
const createTempDir = () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'orbit-git-service-'));
  tempDirs.push(dir);
  return dir;
};
  ...
  it('skips a non-executable post-checkout hook', async () => {
    ...
    try {
      ...
      const created = await createWorktree(repo, {
        ...
        branchName: 'orbit/hook-skip-test',
        returnAfterDirectoryCreated: true,
      });
  ...
  it('does not fail worktree bootstrap when the post-checkout hook fails', async () => {
    ...
    try {
      ...
      const created = await createWorktree(repo, {
        ...
        branchName: 'orbit/hook-fail-test',
        returnAfterDirectoryCreated: true,
      });
