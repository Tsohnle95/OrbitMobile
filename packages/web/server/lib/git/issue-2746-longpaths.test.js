...
// ---------------------------------------------------------------------------
// Regression for https://github.com/openchamber/orbit/issues/2746
//
// "[Bug] new worktree， Filename too long"
//
// Orbit places worktrees under:
...
const createTempDir = () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'orbit-git-issue2746-'));
  tempDirs.push(dir);
  return dir;
};
  ...
  it('surfaces guided bootstrap failure when a path component exceeds the filesystem name limit', async () => {
    ...
    try {
      ...
      const created = await createWorktree(repo, {
        ...
        branchName: 'orbit/issue-2746-namemax',
      });
      expect(created.directoryCreated).toBe(true);
