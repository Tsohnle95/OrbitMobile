...
const createCommandTimeoutMs = () => {
  const raw = Number(process.env.ORBIT_FS_EXEC_TIMEOUT_MS);
...
const createGitReadCacheTtlMs = () => {
  const raw = Number(process.env.ORBIT_GIT_READ_CACHE_TTL_MS);
  if (Number.isFinite(raw) && raw >= 0) return raw;
  return 30 * 1000;
};

const createGitCheckIgnoreTimeoutMs = () => {
  const raw = Number(process.env.ORBIT_GIT_CHECK_IGNORE_TIMEOUT_MS);
  if (Number.isFinite(raw) && raw >= 0) return raw;
  return 2500;
};

const createUploadMaxBytes = () => {
  const raw = Number(process.env.ORBIT_FS_UPLOAD_MAX_BYTES);
...
const resolveWorkspacePath = ({ targetPath, baseDirectory, path, os, normalizeDirectoryPath, orbitUserConfigRoot }) => {
  ...
  if (isPathWithinRoot(resolved, orbitUserConfigRoot, path, os)) {
    return { ok: true, base: path.resolve(orbitUserConfigRoot), resolved };
  }
...
const resolveWorkspacePathFromContext = async ({ req, targetPath, resolveProjectDirectory, path, os, normalizeDirectoryPath, orbitUserConfigRoot }) => {
...
const resolveReadPathFromContext = async ({ req, targetPath, scope, resolveProjectDirectory, path, os, fsPromises, normalizeDirectoryPath, orbitUserConfigRoot }) => {
