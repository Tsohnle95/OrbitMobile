...
import { registerOrbitRoutes } from './lib/opencode/orbit-routes.js';
...
import { createOrbitSessionService } from './lib/orbit-sessions/routes.js';
import { createScheduledTaskService } from './lib/scheduled-tasks/service.js';
import { createOrbitControlService } from './lib/orbit-control/service.js';
import { OrbitControlError } from './lib/orbit-control/error.js';
...
const DESKTOP_NOTIFY_PREFIX = '[OrbitDesktopNotify] ';
const uiNotificationClients = new Set();
const uiNotificationWsClients = new Set();
const uiOrbitEventClients = new Set();
...
const SSE_PATH_PREFIXES = [
  ...
  '/api/orbit/events',
  '/api/orbit/realtime-proxy/sse',
];

function shouldSkipCompression(req, res) {
  if (process.env.ORBIT_RUNTIME === 'desktop') {
    return true;
  }
...
const shouldSkipApiCompression = () => {
  if (isEnvFlagEnabled(process.env.ORBIT_SKIP_API_COMPRESSION)) return true;
  if (isEnvFlagEnabled(process.env.ORBIT_COMPRESS_API)) return false;
  if (isEnvFlagDisabled(process.env.ORBIT_COMPRESS_API)) return true;
  return process.env.ORBIT_RUNTIME === 'desktop';
};

const ORBIT_VERBOSE_REQUEST_LOGS = isEnvFlagEnabled(process.env.ORBIT_VERBOSE_REQUEST_LOGS);
...
const ORBIT_USER_CONFIG_ROOT = path.join(os.homedir(), '.config', 'orbit');
const ORBIT_USER_THEMES_DIR = path.join(ORBIT_USER_CONFIG_ROOT, 'themes');
const ORBIT_PROJECTS_CONFIG_DIR = path.join(ORBIT_USER_CONFIG_ROOT, 'projects');
...
const themeRuntime = createThemeRuntime({
  fsPromises,
  path,
  themesDir: ORBIT_USER_THEMES_DIR,
...
const ORBIT_DATA_DIR = process.env.ORBIT_DATA_DIR
  ? path.resolve(process.env.ORBIT_DATA_DIR)
  : path.join(os.homedir(), '.config', 'orbit');
const SETTINGS_FILE_PATH = path.join(ORBIT_DATA_DIR, 'settings.json');
const PUSH_SUBSCRIPTIONS_FILE_PATH = path.join(ORBIT_DATA_DIR, 'push-subscriptions.json');
const APNS_TOKENS_FILE_PATH = path.join(ORBIT_DATA_DIR, 'apns-tokens.json');
const REMOTE_CLIENTS_FILE_PATH = path.join(ORBIT_DATA_DIR, 'remote-clients.json');
const CLIENT_PAIRING_SESSIONS_FILE_PATH = path.join(ORBIT_DATA_DIR, 'client-pairing-sessions.json');
const CLOUDFLARE_MANAGED_REMOTE_TUNNELS_FILE_PATH = path.join(ORBIT_DATA_DIR, 'cloudflare-managed-remote-tunnels.json');
const CLOUDFLARE_LEGACY_NAMED_TUNNELS_FILE_PATH = path.join(ORBIT_DATA_DIR, 'cloudflare-named-tunnels.json');
...
const projectConfigRuntime = createProjectConfigRuntime({
  fsPromises,
  path,
  projectsDirPath: ORBIT_PROJECTS_CONFIG_DIR,
});
...
const agentMemoryRuntime = createAgentMemoryRuntime({
  ...
  path,
  projectsDirPath: ORBIT_PROJECTS_CONFIG_DIR,
  userConfigRoot: ORBIT_USER_CONFIG_ROOT,
});
...
const ENV_SKIP_OPENCODE_START = process.env.OPENCODE_SKIP_START === 'true' ||
                                    process.env.ORBIT_SKIP_OPENCODE_START === 'true';
const ENV_DESKTOP_NOTIFY = (() => {
  if (process.env.ORBIT_DESKTOP_NOTIFY === 'true') {
    return true;
  }
  ...
  return /orbit-server/i.test(argv0) || /orbit-server/i.test(argv1);
})();
...
const ENV_CONFIGURED_API_PREFIX = normalizeApiPrefix(
  process.env.OPENCODE_API_PREFIX || process.env.ORBIT_API_PREFIX || ''
);
...
const scheduledTasksRuntime = createScheduledTasksRuntime({
  ...
  emitTaskRunEvent: (event) => {
    for (const client of uiOrbitEventClients) {
      try {
        writeSseEvent(client, {
          type: 'orbit:scheduled-task-ran',
          properties: {
        ...
        uiOrbitEventClients.delete(client);
      }
    }
  },
  logger: console,
...
const resolveMemoryProjectId = createMemoryProjectResolver({
  ...
  managedProjectRoots: [path.join(ORBIT_USER_CONFIG_ROOT, 'chats')],
});
...
const emitAgentMemoryChangedEvent = (event) => {
  for (const client of uiOrbitEventClients) {
    try {
      writeSseEvent(client, {
        type: 'orbit:agent-memory-changed',
        properties: {
          scope: event.scope,
          ...(event.projectId ? { projectId: event.projectId } : {}),
        },
...
const orbitSessionService = createOrbitSessionService({
  readSettingsFromDiskMigrated,
  sanitizeProjects,
...
// Browser actions are published to whichever Orbit clients are connected;
...
const browserControlBroker = createBrowserControlBroker({
  ...
  emitRequest: (request) => {
    ...
    let delivered = 0;
    for (const client of uiOrbitEventClients) {
      if (needsBrowserView && client.orbitBrowserCapable !== true) continue;
      try {
        writeSseEvent(client, {
          type: 'orbit:browser-control-request',
          properties: {
...
const orbitControlService = createOrbitControlService({
  readSettingsFromDiskMigrated,
  sanitizeProjects,
  buildOpenCodeUrl,
  getOpenCodeAuthHeaders,
  waitForOpenCodeReady,
  sessionService: orbitSessionService,
  scheduledTaskService,
  browserControl: browserControlBroker,
  agentMemoryActions: createAgentMemoryActions({
    agentMemoryRuntime,
    createError: (message, status) => new OrbitControlError(message, status),
...
async function main(options = {}) {
  ...
  const effectiveBindHost = host
    || (typeof process.env.ORBIT_HOST === 'string' && process.env.ORBIT_HOST.trim().length > 0
      ? process.env.ORBIT_HOST.trim()
      : '127.0.0.1');
  agentToolRuntime = createAgentToolRuntime({
    crypto,
    fsPromises,
    path,
    dataDir: ORBIT_DATA_DIR,
    env: process.env,
    executeAction: (...args) => orbitControlService.execute(...args),
    getActivePort: () => {
  ...
  const uiPassword = typeof options.uiPassword === 'string'
    ? options.uiPassword
    : (typeof process.env.ORBIT_UI_PASSWORD === 'string' ? process.env.ORBIT_UI_PASSWORD : null);
  if (
  ...
  const apiOnly = options.apiOnly === true || isEnvFlagEnabled(process.env.ORBIT_API_ONLY);
  ...
  console.log(`Starting Orbit on port ${port === 0 ? 'auto' : port}`);
  ...
  const bootstrapResult = bootstrapRuntime.setupBaseRoutes(app, {
    process,
    orbitVersion: ORBIT_VERSION,
    runtimeName: process.env.ORBIT_RUNTIME || 'web',
    serverStartedAt,
    gracefulShutdown,
    getHealthSnapshot: () => {
    ...
    verboseRequestLogs: ORBIT_VERBOSE_REQUEST_LOGS,
    uiPassword,
    tunnelAuthController,
    remoteClientAuthRuntime,
    ...
    getServerLabel: () => {
      try {
        ...
        return typeof name === 'string' && name.trim().length > 0 ? name.trim() : 'Orbit';
      } catch {
        return 'Orbit';
      }
    },
  ...
  const relayService = createRelayService({
    ...
    hostLock: createRelayHostLock({
      lockFilePath: path.join(ORBIT_DATA_DIR, 'relay-host.lock'),
      fs,
      process,
    }),
    ...
    // paired devices would land on them. ORBIT_RELAY_HOST=off disables
    // passive hosting explicitly (dev scripts set it); the Electron dev shell is
    // covered via ORBIT_ELECTRON_DEV. ORBIT_RELAY_HOST=on overrides
    // both. Explicit enable/pairing on the instance still hosts regardless.
    allowPassiveHost: process.env.ORBIT_RELAY_HOST === 'on'
      || (process.env.ORBIT_RELAY_HOST !== 'off' && process.env.ORBIT_ELECTRON_DEV !== '1'),
  ...
  await featureRoutesRuntime.registerRoutes(app, {
    ...
    orbitDataDir: ORBIT_DATA_DIR,
    orbitUserConfigRoot: ORBIT_USER_CONFIG_ROOT,
    normalizeDirectoryPath,
    resolveProjectDirectory,
    ...
    getOpenCodeAuthHeaders,
    getOpenCodePort: () => openCodePort,
    // Dev-server discovery must not offer Orbit's own listeners back to
    ...
    waitForOpenCodeReady,
    emitSessionCreatedEvent,
    getOrbitEventClients: () => uiOrbitEventClients,
    writeSseEvent,
  ...
  const startupPipelineResult = await startupPipelineRuntime.run({
    ...
    attachSignals,
    apiOnly,
    dictationModelsDir: path.join(ORBIT_USER_CONFIG_ROOT, 'speech-models'),
  });
  ...
  // Relay demand can change outside our routes: `orbit connect-url
