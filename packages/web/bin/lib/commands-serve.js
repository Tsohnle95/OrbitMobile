...
import { discoverOrbitInstanceOnPort, isDesktopRuntimeForPort } from './cli-lifecycle.js';
...
async function serveCommand(options) {
    ...
    if (targetPort !== 0 && !options.suppressUnsafePortWarning) {
      assertSafeBrowserPort(targetPort, { context: 'Orbit serve' });
    }

    if (targetPort !== 0) {
      const existingInstance = await discoverOrbitInstanceOnPort(targetPort, { host: effectiveHost });
      if (existingInstance?.runtime === 'desktop') {
        throw new Error(
          `Port ${targetPort} is used by Orbit Desktop app. Choose another port or stop the desktop app.`
        );
      }
      if (existingInstance) {
        const pidSuffix = Number.isFinite(existingInstance.pid) ? ` (PID: ${existingInstance.pid})` : '';
        if (existingInstance.source === 'probe') {
          throw new Error(`Orbit is already running on port ${targetPort}. Use \`orbit status\` or \`orbit stop --port ${targetPort}\`.`);
        }
        throw new Error(`Orbit is already running on port ${targetPort}${pidSuffix}`);
      }
    ...
    if (!effectiveUiPassword && !options.suppressUiPasswordWarning) {
      ...
      const warningLine = 'ORBIT_UI_PASSWORD is not set';
      const warningDetail = networkExposed
        ? `server is bound to ${bindHost} and reachable on your network with no UI auth. `
          + 'Set --ui-password or ORBIT_UI_PASSWORD before exposing it over LAN.'
        : 'browser UI is unsecured. Use --ui-password or ORBIT_UI_PASSWORD.';
      if (showOutput) {
    ...
    if (options.foreground) {
      ...
      if (effectiveUiPassword) {
        process.env.ORBIT_UI_PASSWORD = effectiveUiPassword;
      }
      process.env.ORBIT_HOST = effectiveHost;
      process.env.ORBIT_RUNTIME = 'web';
      ...
      if (!isQuietMode(options)) {
        console.log(`Starting Orbit on port ${targetPort === 0 ? 'auto' : targetPort} (foreground)`);
      }
    ...
    const child = spawn(runtimeBin, serverArgs, {
      ...
      env: {
        ...process.env,
        ORBIT_PORT: String(targetPort),
        ORBIT_RUNTIME: 'web',
        OPENCODE_BINARY: opencodeBinary,
        ORBIT_HOST: effectiveHost,
        ...(effectiveUiPassword ? { ORBIT_UI_PASSWORD: effectiveUiPassword } : {}),
        ...(options.apiOnly === true ? { ORBIT_API_ONLY: 'true' } : {}),
        ...(process.env.OPENCODE_SKIP_START ? { ORBIT_SKIP_OPENCODE_START: process.env.OPENCODE_SKIP_START } : {}),
      },
    });

    child.unref();
    serveSpin?.start(`Starting Orbit on port ${targetPort === 0 ? 'auto' : targetPort}...`);
    ...
    try {
      resolvedPort = await new Promise((resolve, reject) => {
        ...
        const timeout = setTimeout(() => {
          if (settled) return;
          settled = true;
          reject(new Error(`Orbit daemon did not report ready within ${DAEMON_READY_TIMEOUT_MS / 1000}s`));
        }, DAEMON_READY_TIMEOUT_MS);

        child.on('message', (msg) => {
          if (settled) return;
          if (msg && msg.type === 'orbit:ready' && typeof msg.port === 'number') {
            settled = true;
        ...
        child.on('exit', (code, signal) => {
          ...
          settled = true;
          clearTimeout(timeout);
          reject(new Error(`Orbit daemon exited before reporting ready${signal ? ` (${signal})` : ` (code ${code ?? 'unknown'})`}`));
        });
      });
    } catch (error) {
    ...
    if (!isProcessRunning(child.pid)) {
      serveSpin?.error('Failed to start Orbit');
    ...
    const serveResult = {
      ...
      logs: `orbit logs -p ${resolvedPort}`,
      launchMode: 'daemon',
    };
    ...
    if (isQuietMode(options)) {
      ...
      // in the same compact `pass:` token form `orbit status --quiet`
