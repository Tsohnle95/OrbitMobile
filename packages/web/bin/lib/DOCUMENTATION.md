...
This directory contains the non-entrypoint implementation for the Orbit CLI. `packages/web/bin/cli.js` should stay thin: it owns bootstrap, command wiring, top-level dispatch, signal/cancel handling, and compatibility exports. Domain logic belongs in these modules.
...
- `commands-lifecycle.js`
  - Implements `orbit stop` and `orbit restart`.
...
- `commands-status.js`
  - Implements `orbit status`.
  - Formats discovered instances and tunnel readiness/status for human, quiet, and JSON output.

- `commands-session.js`
  - Implements `orbit session create`, `send`, `fork`, `list`, `status`, and `messages`.
...
- `commands-connect-url.js`
  ...
  - `--relay` builds a relay-only pairing link (the sole candidate is the relay transport), for sharing with a device that is not on the host's network — no server URL, no auto-start. The relay endpoint follows `ORBIT_RELAY_URL` / the stored setting / the default, matching the running host; the host must be running with the relay enabled to serve the redeem over the tunnel.

- `commands-update.js`
  - Implements `orbit update`.
  - Loads the package-manager helper, performs update flow, and coordinates restart behavior after updates.

- `commands-tunnel.js`
  - Implements `orbit tunnel` and its subcommands: `profile`, `providers`, `ready`, `doctor`, `status`, `start`, `stop`, and `completion`.
...
- `cli-control.js`
  - Sends one typed action request to the authenticated Orbit control endpoint and maps HTTP failures to CLI exit behavior.
  - Must not reproduce session, scheduled-task, project-resolution, or wait orchestration.

- `cli-api-target.js`
  - Resolves the target Orbit runtime for control-plane commands, preferring desktop unless a port is explicit.
