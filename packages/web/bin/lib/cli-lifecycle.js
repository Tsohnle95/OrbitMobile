...
function createLivePortInstance(port, info, host) {
  if (!hasOrbitRuntimeInfo(info)) return null;
  return {
    port,
    pid: Number.isFinite(info.pid) ? info.pid : null,
    pidFilePath: path.join(getRunDir(), `orbit-${port}.pid`),
    instanceFilePath: path.join(getRunDir(), `orbit-${port}.json`),
    mtime: 0,
    startedAt: 0,
    launchMode: 'daemon',
...
async function resolveDoctorPortStatuses(options = {}) {
  ...
  if (options.explicitPort) {
    ...
    if (desktopEntry && desktopEntry.port === requestedPort) {
      statuses.push({
        ...
        detail: 'Use a CLI instance port from `orbit serve` for tunneling.',
      });
    ...
    statuses.push({
      ...
      detail: `Start one with \`orbit serve --port ${requestedPort}\`.`,
    });
    return { statuses, availableEntries: [] };
  }
  ...
  if (runningEntries.length === 0) {
    statuses.push({
      ...
      detail: 'Start one with `orbit serve`.',
    });
  }
...
async function discoverRunningInstances(options = {}) {
  ...
  const getProcessState = typeof options.getOrbitProcessState === 'function'
    ? options.getOrbitProcessState
    : (pid) => getOrbitProcessState(pid, options);
  try {
    const files = fs.readdirSync(runDir);
    const pidFiles = files.filter((file) => file.startsWith('orbit-') && file.endsWith('.pid'));
    for (const file of pidFiles) {
      const port = parseInt(file.replace('orbit-', '').replace('.pid', ''), 10);
      if (!Number.isFinite(port) || port <= 0) continue;
      const pidFilePath = path.join(runDir, file);
      const pid = readPidFile(pidFilePath);
      if (!pid) {
        removePidFile(pidFilePath);
        removeInstanceFile(path.join(runDir, `orbit-${port}.json`));
        continue;
      }

      const instanceFilePath = path.join(runDir, `orbit-${port}.json`);
      ...
      // confirms Orbit. Cmdline identity alone can match a recycled PID
      // from another Orbit process on a different port. Try all plausible
      ...
      if (!hasOrbitRuntimeInfo(liveInfo)) {
...
async function discoverOrbitInstanceOnPort(port, options = {}) {
...
async function discoverLifecycleInstances(options = {}, deps = {}) {
  ...
  const liveInstance = await discoverOrbitInstanceOnPort(options.port, {
    ...deps,
    host: options.host,
    runningInstances,
  });
...
async function discoverUnconfirmedRegistryInstanceOnPort(port, options = {}) {
  ...
  const storedOptions = readInstanceOptions(instanceFilePath);
  const processState = getOrbitProcessState(pid);
  if (processState === 'dead') {
    removePidFile(pidFilePath);
    removeInstanceFile(instanceFilePath);
    return null;
