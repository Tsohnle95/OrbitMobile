...
async function resolveTargetPort(options = {}, deps = {}) {
  ...
  if (ports.length > 1) {
    ...
    throw new TunnelCliError(
      `Multiple Orbit instances are running (ports: ${ports.join(', ')}). Choose one with --port <port>.`,
  ...
  throw new TunnelCliError(
    'No running Orbit server found. Start one with `orbit serve`, or pass --port <port>.',
