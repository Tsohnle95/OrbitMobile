...
function parseArgs(argv = process.argv.slice(2)) {
  ...
  const options = {
    port: DEFAULT_PORT,
    host: undefined,
    uiPassword: process.env.ORBIT_UI_PASSWORD || undefined,
    json: false,
    all: false,
    follow: true,
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    switch (name) {
      ...
      case 'd':
        ...
        break;
      case 'try-cf-tunnel':
        removedFlagErrors.push('`--try-cf-tunnel` was removed. Use: orbit tunnel start --provider cloudflare --mode quick');
        break;
      case 'tunnel-qr':
        removedFlagErrors.push('`--tunnel-qr` was removed. Use: orbit tunnel start ... --qr');
        break;
      ...
      case 'tunnel':
        removedFlagErrors.push(`\`--${name}\` was removed from top-level serve flow. Use: orbit tunnel start ...`);
        break;
      default:
...
function showHelp() {
  console.log(`
 Orbit - Web interface for the OpenCode AI coding agent
...
ENVIRONMENT:
  ORBIT_HOST             Bind address (e.g. 0.0.0.0 for all interfaces)
  ORBIT_UI_PASSWORD      Alternative to --ui-password flag
  ORBIT_API_ONLY         Set to true/1 to start API routes only
  ORBIT_DATA_DIR         Override Orbit data directory
  ...
  ORBIT_OPENCODE_HOSTNAME  Bind hostname for managed OpenCode server (default: 127.0.0.1)

EXAMPLES:
  orbit                    # Start in daemon mode on default port 3000 (or free port)
  orbit --port 8080        # Start on port 8080 (daemon)
  orbit --lan --port 3002  # Start on LAN at 0.0.0.0:3002
  orbit serve --foreground # Start in foreground (for systemd Type=simple)
  orbit connect-url --port 3000 --qr
  orbit connect-url --server https://orbit.example.com
  orbit control           # Show control-plane commands for agents/scripts
  orbit startup enable     # Start Orbit at user login
  orbit tunnel help        # Show tunnel lifecycle help
  orbit logs               # Follow logs for latest running instance
`);
}

function showControlHelp() {
  console.log(`
 Orbit Control Commands

USAGE:
  orbit <COMMAND> [OPTIONS]

COMMANDS:
  status                         Show running Orbit runtimes
...
DETAILED HELP:
  orbit session --help     Show session creation, status, and message options
  orbit models --help      Show model defaults and favorites help
  orbit projects --help    Show project list help
  orbit schedule --help    Show scheduled task actions and schedule options
  orbit tunnel help        Show tunnel lifecycle/status commands
  orbit status --help      Show runtime status options

COMMON OPTIONS:
  --json                         Output machine-readable JSON
  -q, --quiet                    Print minimal output
  -p, --port <port>              Target a specific Orbit runtime
...
function showStartupHelp() {
  console.log(`
 Orbit Startup Commands

USAGE:
  orbit startup <SUBCOMMAND> [OPTIONS]
...
EXAMPLES:
  orbit startup enable
  orbit startup enable --port 3000
  orbit startup enable --port 3000 --api-only --host 0.0.0.0
  orbit startup status --json
`);
}
...
USAGE:
  orbit connect-url [OPTIONS]

DESCRIPTION:
  Generate an orbit:// connection link for adding this server to another
  Orbit app. If no server is running on the selected port, it starts one.
...
OPTIONS:
  ...
  --relay                 Also include the end-to-end-encrypted relay transport
                          ...
                          ORBIT_RELAY_URL to use a self-hosted relay.
...
EXAMPLES:
  orbit connect-url --port 3000 --qr
  orbit connect-url --port 3000 --api-only --lan --server http://workstation.local:3000 --qr
  orbit connect-url --server https://orbit.example.com --name Workstation
  orbit connect-url --relay --name "My laptop"
`);
}
...
USAGE:
  orbit tunnel <SUBCOMMAND> [OPTIONS]
...
COMMON OPTIONS:
  -p, --port              Target Orbit instance port
...
BEHAVIOR NOTES:
  - One active tunnel per Orbit instance.
...
PROFILE USAGE:
  orbit tunnel profile list [--provider <id>] [--json]
  orbit tunnel profile show --name <name> [--provider <id>] [--json]
  orbit tunnel profile add --provider <id> --mode managed-remote --name <name> --hostname <host> --token <token> [--force] [--json]
  orbit tunnel profile add --provider <id> --mode managed-remote --name <name> --hostname <host> --token-file <path> [--force] [--json]
  orbit tunnel profile remove --name <name> [--provider <id>] [--json]

SHELL COMPLETION:
  orbit tunnel completion bash   Generate Bash completion script
  orbit tunnel completion zsh    Generate Zsh completion script
  orbit tunnel completion fish   Generate Fish completion script

EXAMPLES:
  orbit tunnel providers
  orbit tunnel ready --provider cloudflare
  orbit tunnel doctor --provider cloudflare
  orbit tunnel status
  orbit tunnel start --qr
  orbit tunnel start --profile prod-main
  orbit tunnel start --provider cloudflare --mode managed-remote --token-file ~/.secrets/cf-token --hostname app.example.com
  orbit tunnel start --provider cloudflare --mode managed-local --config ~/.cloudflared/config.yml
  orbit tunnel start --dry-run --provider cloudflare --mode managed-remote --token-file ~/.secrets/cf-token --hostname app.example.com
  echo "$TOKEN" | orbit tunnel profile add --provider cloudflare --mode managed-remote --name prod-main --hostname app.example.com --token-stdin
  orbit tunnel profile list --provider cloudflare
  orbit tunnel profile list --json --show-secrets
  orbit tunnel stop --port 3000
`);
}
...
function generateCompletionScript(shell) {
  ...
  if (normalized === 'bash') {
    return `# Bash completion for orbit tunnel
# Add to ~/.bashrc: eval "$(orbit tunnel completion bash)"
_orbit_tunnel() {
...
complete -F _orbit_tunnel orbit
`;
  }

  if (normalized === 'zsh') {
    return `#compdef orbit
# Zsh completion for orbit tunnel
# Add to ~/.zshrc: eval "$(orbit tunnel completion zsh)"
...
`;
  ...
  if (normalized === 'fish') {
    return `# Fish completion for orbit tunnel
# Save to ~/.config/fish/completions/orbit.fish

complete -c orbit -n '__fish_use_subcommand' -a 'serve' -d 'Start the web server'
complete -c orbit -n '__fish_seen_subcommand_from serve' -l foreground -d 'Run in foreground (for systemd/process managers)'
complete -c orbit -n '__fish_seen_subcommand_from serve' -l no-daemon -d 'Run in foreground (alias for --foreground)'
complete -c orbit -n '__fish_use_subcommand' -a 'stop' -d 'Stop running instance(s)'
complete -c orbit -n '__fish_use_subcommand' -a 'restart' -d 'Stop and start the server'
complete -c orbit -n '__fish_use_subcommand' -a 'status' -d 'Show server status'
complete -c orbit -n '__fish_use_subcommand' -a 'tunnel' -d 'Tunnel lifecycle commands'
complete -c orbit -n '__fish_use_subcommand' -a 'logs' -d 'Tail logs'
complete -c orbit -n '__fish_use_subcommand' -a 'update' -d 'Check for updates'

complete -c orbit -n '__fish_seen_subcommand_from tunnel; and not __fish_seen_subcommand_from help providers ready doctor status start stop profile completion' -a 'help' -d 'Show tunnel help'
complete -c orbit -n '__fish_seen_subcommand_from tunnel; and not __fish_seen_subcommand_from help providers ready doctor status start stop profile completion' -a 'providers' -d 'Show providers'
complete -c orbit -n '__fish_seen_subcommand_from tunnel; and not __fish_seen_subcommand_from help providers ready doctor status start stop profile completion' -a 'ready' -d 'Check readiness'
complete -c orbit -n '__fish_seen_subcommand_from tunnel; and not __fish_seen_subcommand_from help providers ready doctor status start stop profile completion' -a 'doctor' -d 'Run diagnostics'
complete -c orbit -n '__fish_seen_subcommand_from tunnel; and not __fish_seen_subcommand_from help providers ready doctor status start stop profile completion' -a 'status' -d 'Show tunnel status'
complete -c orbit -n '__fish_seen_subcommand_from tunnel; and not __fish_seen_subcommand_from help providers ready doctor status start stop profile completion' -a 'start' -d 'Start a tunnel'
complete -c orbit -n '__fish_seen_subcommand_from tunnel; and not __fish_seen_subcommand_from help providers ready doctor status start stop profile completion' -a 'stop' -d 'Stop tunnel'
complete -c orbit -n '__fish_seen_subcommand_from tunnel; and not __fish_seen_subcommand_from help providers ready doctor status start stop profile completion' -a 'profile' -d 'Manage profiles'
complete -c orbit -n '__fish_seen_subcommand_from tunnel; and not __fish_seen_subcommand_from help providers ready doctor status start stop profile completion' -a 'completion' -d 'Generate completions'

complete -c orbit -n '__fish_seen_subcommand_from tunnel; and __fish_seen_subcommand_from start' -l provider -d 'Provider id'
complete -c orbit -n '__fish_seen_subcommand_from tunnel; and __fish_seen_subcommand_from start' -l mode -d 'Tunnel mode'
complete -c orbit -n '__fish_seen_subcommand_from tunnel; and __fish_seen_subcommand_from start' -l profile -d 'Profile name'
complete -c orbit -n '__fish_seen_subcommand_from tunnel; and __fish_seen_subcommand_from start' -l config -d 'Config path'
complete -c orbit -n '__fish_seen_subcommand_from tunnel; and __fish_seen_subcommand_from start' -l token -d 'Token'
complete -c orbit -n '__fish_seen_subcommand_from tunnel; and __fish_seen_subcommand_from start' -l token-file -d 'Token file path'
complete -c orbit -n '__fish_seen_subcommand_from tunnel; and __fish_seen_subcommand_from start' -l token-stdin -d 'Read token from stdin'
complete -c orbit -n '__fish_seen_subcommand_from tunnel; and __fish_seen_subcommand_from start' -l hostname -d 'Hostname'
complete -c orbit -n '__fish_seen_subcommand_from tunnel; and __fish_seen_subcommand_from start' -l dry-run -d 'Validate without applying'
complete -c orbit -n '__fish_seen_subcommand_from tunnel; and __fish_seen_subcommand_from start' -l qr -d 'Show QR code'
`;
  }
