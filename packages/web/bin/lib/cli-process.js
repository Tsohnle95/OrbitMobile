...
async function getPidFilePath(port) {
  return path.join(getRunDir(), `orbit-${port}.pid`);
}

async function getInstanceFilePath(port) {
  return path.join(getRunDir(), `orbit-${port}.json`);
}
...
// that PID to an unrelated process — see isOrbitProcessRunning.
...
function isOrbitCmdline(cmdline) {
  if (typeof cmdline !== 'string' || cmdline.length === 0) {
    return false;
  }
  // Every install path contains the "orbit" segment — the npm package
  // (@orbit/web) and the source checkout both do, for the foreground
  ...
  return cmdline.toLowerCase().includes('orbit');
}

// Liveness + identity — "is the Orbit instance recorded in a pid file
...
function isOrbitProcessRunning(pid) {
  const state = getOrbitProcessState(pid);
  return state === 'matched' || state === 'unknown';
}

function getOrbitProcessState(pid, options = {}) {
  ...
  return isOrbitCmdline(cmdline) ? 'matched' : 'mismatched';
}

function hasOrbitRuntimeInfo(info) {
