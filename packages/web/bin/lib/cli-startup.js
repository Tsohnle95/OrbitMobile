...
const STARTUP_SERVICE_ID = 'dev.orbit.web';
...
function getStartupServicePaths() {
  ...
  if (process.platform === 'linux') {
    return {
      platform: 'linux',
      servicePath: path.join(os.homedir(), '.config', 'systemd', 'user', 'orbit.service'),
    };
  }
...
function getMacosStartupWrapperPath() {
  return path.join(getDataDir(), 'bin', 'Orbit');
}
...
function collectStartupEnv(options = {}) {
  ...
  if (uiPassword) {
    env.ORBIT_UI_PASSWORD = uiPassword;
  }
  if (options.apiOnly === true) {
    env.ORBIT_API_ONLY = 'true';
  }
  if (typeof process.env.ORBIT_DATA_DIR === 'string' && process.env.ORBIT_DATA_DIR.trim().length > 0) {
    env.ORBIT_DATA_DIR = path.resolve(process.env.ORBIT_DATA_DIR.trim());
  }
  return env;
}
...
function buildMacosLaunchAgent(options = {}) {
  ...
  const env = collectStartupEnv(options);
  const logDir = path.join(os.homedir(), 'Library', 'Logs', 'Orbit');
...
function getStartupStatus() {
  ...
  if (paths.platform === 'linux') {
    const enabledResult = runStartupCommand('systemctl', ['--user', 'is-enabled', 'orbit.service'], { allowFailure: true });
    const activeResult = runStartupCommand('systemctl', ['--user', 'is-active', 'orbit.service'], { allowFailure: true });
...
function enableStartupService(options = {}) {
  ...
  if (paths.platform === 'macos') {
    ...
    fs.mkdirSync(path.join(os.homedir(), 'Library', 'Logs', 'Orbit'), { recursive: true, mode: 0o700 });
  ...
  if (paths.platform === 'linux') {
    ...
    runStartupCommand('systemctl', ['--user', 'enable', '--now', 'orbit.service']);
    return getStartupStatus();
  }
...
function disableStartupService() {
  ...
  if (paths.platform === 'linux') {
    runStartupCommand('systemctl', ['--user', 'disable', '--now', 'orbit.service'], { allowFailure: true });
    try { fs.unlinkSync(paths.servicePath); } catch {}
