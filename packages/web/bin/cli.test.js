...
import {
  ...
  getPidFilePath,
  isOrbitCmdline,
  isOrbitProcessRunning,
  parseArgs,
  resolveServeHost,
  resolveServeUiPassword,
} from './cli.js';

async function withTempOrbitDataDir(fn) {
  const previous = process.env.ORBIT_DATA_DIR;
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'orbit-cli-test-'));
  process.env.ORBIT_DATA_DIR = dir;
  try {
    return await fn(dir);
  } finally {
    if (typeof previous === 'string') {
      process.env.ORBIT_DATA_DIR = previous;
    } else {
...
async function startMockOrbitServer(options = {}) {
...
function spawnOrbitLikeIdleProcess() {
  return spawn(process.execPath, ['-e', 'setInterval(() => {}, 1000)', 'orbit-idle'], { stdio: 'ignore' });
}

function spawnOrbitLikeHungServer(port) {
  const script = `
    ...
    setInterval(() => {}, 1000);
  `;
  return spawn(process.execPath, ['-e', script, 'orbit-hung-server'], { stdio: 'ignore' });
}
...
describe('cli args', () => {
  ...
  it('parses explicit connect-url server overrides', () => {
    const parsed = parseArgs(['connect-url', '--server', 'https://orbit.example.com', '--port', '3002']);

    expect(parsed.command).toBe('connect-url');
    expect(parsed.options.server).toBe('https://orbit.example.com');
  ...
  it('parses session create options', () => {
    ...
    expect(parsed.options.branch).toBe('orbit/side-task');
  ...
  it('formats projects compactly', () => {
    expect(formatProjectLine({
      ...
      label: 'Orbit',
      path: '/repo/orbit',
    })).toBe('- `Orbit` — `path_repo` — `/repo/orbit`');
  });
...
describe('cli API target resolution', () => {
  ...
  it('fails when multiple non-default instances are running', async () => {
    await expect(resolveTargetPort({}, {
      ...
      isServerHealthReady: async () => false,
    })).rejects.toThrow('Multiple Orbit instances are running');
  });
});
...
describe('network-exposed auth validation', () => {
  ...
  it('allows explicit unsafe LAN override from process env only', () => {
    const previous = process.env.ORBIT_ALLOW_UNAUTHENTICATED_LAN;
    process.env.ORBIT_ALLOW_UNAUTHENTICATED_LAN = 'true';
    try {
      expect(() => assertAuthenticatedNetworkExposure({ host: '0.0.0.0' })).not.toThrow();
    } finally {
      if (typeof previous === 'string') {
        process.env.ORBIT_ALLOW_UNAUTHENTICATED_LAN = previous;
      } else {
        delete process.env.ORBIT_ALLOW_UNAUTHENTICATED_LAN;
      }
    }
  });
});
...
describe('serve host resolution', () => {
  it('uses ORBIT_HOST when --host is not provided', () => {
    const previous = process.env.ORBIT_HOST;
    process.env.ORBIT_HOST = '192.0.2.20';
    try {
      expect(resolveServeHost(undefined)).toBe('192.0.2.20');
    } finally {
      if (typeof previous === 'string') {
        process.env.ORBIT_HOST = previous;
      } else {
        delete process.env.ORBIT_HOST;
      }
    }
  });

  it('prefers explicit --host over ORBIT_HOST', () => {
    const previous = process.env.ORBIT_HOST;
    process.env.ORBIT_HOST = '192.0.2.20';
    try {
...
describe('compatibility exports', () => {
  it('allows tunnel profile migration before command options are initialized', async () => {
    await withTempOrbitDataDir(async () => {
...
describe('CLI HTTP helpers', () => {
  it('sends one typed request to the shared control endpoint', async () => {
    ...
    globalThis.fetch = async (url, options = {}) => {
      expect(new URL(String(url)).pathname).toBe('/api/orbit/control');
  ...
  it('retries UI-authenticated API requests with the stored instance password', async () => {
    await withTempOrbitDataDir(async () => {
      ...
      try {
        const { response, body } = await requestJson(port, '/api/orbit/tunnel/start', {
          method: 'POST',
  ...
  it('prefers the stored instance password over a non-explicit env password', async () => {
    await withTempOrbitDataDir(async () => {
      ...
      try {
        const { response, body } = await requestJson(port, '/api/orbit/scheduled-tasks/status', {
  ...
  it('authenticates desktop-local API requests with the stored client token', async () => {
    await withTempOrbitDataDir(async (dir) => {
      const port = 57123;
...
describe('cli entry detection', () => {
  const modulePath = '/tmp/orbit/bin/cli.js';
  const moduleUrl = pathToFileURL(modulePath).href;

  it('resolves symlinked entry paths before comparing', () => {
    const symlinkPath = '/usr/local/bin/orbit';
  ...
  it('accepts wrapper binary name fallback when requested', () => {
    const wrapperPath = '/home/user/.local/bin/orbit';
    expect(isModuleCliExecution(wrapperPath, moduleUrl, undefined, 'orbit')).toBe(true);
  });
...
describe('isOrbitCmdline', () => {
  it('accepts Orbit CLI and daemon cmdlines', () => {
    expect(isOrbitCmdline('node /x/@orbit/web/bin/cli.js serve')).toBe(true);
    expect(isOrbitCmdline('node /x/@orbit/web/server/index.js --port 9090')).toBe(true);
    expect(isOrbitCmdline('bun /home/u/projects/orbit/packages/web/server/index.js --port 3001')).toBe(true);
  });

  it('rejects recycled and unrelated processes (issue #1721)', () => {
    expect(isOrbitCmdline('node /home/herjarsa/npm-global/bin/agentmemory')).toBe(false);
    expect(isOrbitCmdline('node /usr/lib/node_modules/npm/bin/npm-cli.js install')).toBe(false);
    expect(isOrbitCmdline('')).toBe(false);
    expect(isOrbitCmdline(null)).toBe(false);
  });
});

describe('isOrbitProcessRunning', () => {
  it('returns false for a dead PID', () => {
    expect(isOrbitProcessRunning(2147483646)).toBe(false);
  });
  ...
  it.skipIf(process.platform !== 'linux' && process.platform !== 'darwin')(
    'returns false for a live non-Orbit PID',
    async () => {
      const child = spawn('sleep', ['30'], { stdio: 'ignore' });
      try {
        await new Promise((resolve) => setTimeout(resolve, 150));
        expect(isOrbitProcessRunning(child.pid)).toBe(false);
      } finally {
        child.kill('SIGKILL');
      }
    }
...
describe('lifecycle instance discovery', () => {
  it('does not attribute a desktop runtime response to a different explicit port', async () => {
    await withTempOrbitDataDir(async (dir) => {
      ...
      const instance = await discoverOrbitInstanceOnPort(3003, {
  ...
  it('attributes a desktop runtime response to its configured desktop port', async () => {
    await withTempOrbitDataDir(async (dir) => {
      ...
      const instance = await discoverOrbitInstanceOnPort(57123, {
        fetchImpl: async () => createMockJsonResponse({ runtime: 'desktop', pid: 934 }),
      });
  ...
  it('keeps pid and instance files when live port probe confirms a cmdline mismatch', async () => {
    await withTempOrbitDataDir(async () => {
      ...
      const instances = await discoverRunningInstances({
        ...
        getOrbitProcessState: () => 'mismatched',
      });
  ...
  it('uses the live system-info pid instead of a stale Orbit-looking pid-file pid', async () => {
    await withTempOrbitDataDir(async () => {
      const port = 45127;
  ...
  it('discovers an explicit live Orbit port without a pid-file registry entry', async () => {
    await withTempOrbitDataDir(async () => {
      const port = 45125;
      const instances = await discoverLifecycleInstances(
  ...
  it('cleans a matched pid-file entry without stopping it when the recorded port is free', async () => {
    await withTempOrbitDataDir(async () => {
      const port = await allocateLoopbackPort();
      const child = spawnOrbitLikeIdleProcess();
      const pidFile = await getPidFilePath(port);
      const instanceFile = await getInstanceFilePath(port);
      try {
        await new Promise((resolve) => setTimeout(resolve, 150));
...
describe('lifecycle commands with unmanaged explicit ports', () => {
  it('serve refuses to start on a live Orbit port without requiring pid files', async () => {
    await withTempOrbitDataDir(async () => {
      const server = await startMockOrbitServer();
      try {
  ...
  it('stop --port can recover a matched pid-file instance whose HTTP endpoint is unresponsive', async () => {
    await withTempOrbitDataDir(async () => {
      const port = await allocateLoopbackPort();
      const child = spawnOrbitLikeHungServer(port);
      const pidFile = await getPidFilePath(port);
      const instanceFile = await getInstanceFilePath(port);
      try {
  ...
  it('plain stop ignores a stale CLI registry entry that resolves to desktop runtime', async () => {
    await withTempOrbitDataDir(async () => {
      const server = await startMockOrbitServer({ runtime: 'desktop' });
      const child = spawn('sleep', ['30'], { stdio: 'ignore' });
