...
- App id / package: `com.orbit.mobile`; app name `Orbit`.
...
- **Deep links** — `orbit://` URL scheme; a reusable intent vocabulary (`apps/deepLinks.ts`)
...
- **iOS widgets + Control Center + Notification Service Extension** — WidgetKit extension
  (`OrbitWidget`), a Control Center control, and an NSE (`OrbitNotificationService`)
  that refreshes widgets from push. All share the App Group `group.com.orbit.mobile`.
...
- Extensions: `OrbitWidget` (WidgetKit, deployment 17.0) and `OrbitNotificationService`
  (NSE, 15.5), both hand-wired into `App.xcodeproj/project.pbxproj` and embedded via a copy phase.
- App Group `group.com.orbit.mobile` in all three targets' entitlements (app + widget + NSE).
- `Info.plist`: `CFBundleURLTypes` scheme `orbit`, `NSCameraUsageDescription`.
- Push entitlement (aps-environment) required.
- APNs `mutable-content: 1` (set server/relay side) wakes the NSE to refresh widgets.

### Android (`android/app`)

- `google-services.json` (committed; Firebase project `orbit-8bf7e`). The Google Services
...
- Apple Developer account; App IDs for the app **and** both extensions
  (`com.orbit.mobile`, `.OrbitWidget`, `.OrbitNotificationService`), each enabled for
