...
enum WidgetStore {
    static let appGroup = "group.com.orbit.mobile"
...
enum WidgetDeepLink {
    static func newSession() -> URL { URL(string: "orbit://new")! }
    static func attention() -> URL { URL(string: "orbit://sessions?filter=attention")! }
    static func status() -> URL { URL(string: "orbit://status")! }
    static func settings() -> URL { URL(string: "orbit://settings")! }
    static func changes() -> URL { URL(string: "orbit://changes")! }
    static func files() -> URL { URL(string: "orbit://view/files")! }
    static func instances() -> URL { URL(string: "orbit://view/instances")! }
    static func session(_ id: String) -> URL {
        let encoded = id.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? id
        return URL(string: "orbit://session/\(encoded)") ?? newSession()
    }
}
...
// MARK: - Logo (full Orbit mark drawn from the SVG)

/// The Orbit logo, drawn to match packages/web/public/logo-dark-512x512.svg: an
