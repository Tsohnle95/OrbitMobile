...
// Control Center control (iOS 18+): tap the Orbit logo to start a new session.
//
...
@available(iOS 18.0, *)
struct OrbitNewSessionControl: ControlWidget {
    var body: some ControlWidgetConfiguration {
        StaticControlConfiguration(kind: "OrbitNewSessionControl") {
        ...
        .description("Start a new Orbit session.")
    }
}

@available(iOS 18.0, *)
struct OpenNewSessionIntent: AppIntent {
    static let title: LocalizedStringResource = "New Orbit Session"
    static let openAppWhenRun: Bool = true
    static let isDiscoverable: Bool = true

    @MainActor
    func perform() async throws -> some IntentResult & OpensIntent {
        return .result(opensIntent: OpenURLIntent(URL(string: "orbit://new")!))
    }
}
