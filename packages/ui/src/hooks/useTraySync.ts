...
const TRAY_ACTION_EVENT = 'orbit:tray-action';
...
type TraySnapshot = {
  ...
  // Active instance label (e.g. "Local Orbit" or a remote host name) so
...
// the existing `orbit:open-session` / `orbit:open-draft-session`
...
const isTrayPlatform = (): boolean => {
  ...
  const platform = (window as unknown as { __ORBIT_PLATFORM__?: string }).__ORBIT_PLATFORM__;
  return platform === 'darwin' || platform === 'win32' || platform === 'linux';
};

const isTrayEnabled = (): boolean =>
  typeof window !== 'undefined' && window.__ORBIT_ELECTRON__?.trayEnabled !== false;
...
// the local origin shows as "Local Orbit"; a remote host shows its
// configured name. Async because the host config is read over IPC.
const resolveInstanceName = async (): Promise<string> => {
  try {
    if (isDesktopLocalOriginActive()) return 'Local Orbit';
    const localOrigin = (window as unknown as { __ORBIT_LOCAL_ORIGIN__?: string }).__ORBIT_LOCAL_ORIGIN__
      || window.location.origin;
    const runtimeApiBaseUrl = getRuntimeApiBaseUrl();
    if (runtimeApiBaseUrl && locationMatchesHost(runtimeApiBaseUrl, localOrigin)) return 'Local Orbit';
...
export const useTraySync = (): void => {
  ...
  React.useEffect(() => {
    ...
    const bridge = (window as unknown as { __ORBIT_DESKTOP__?: DesktopBridgeGlobal }).__ORBIT_DESKTOP__;
