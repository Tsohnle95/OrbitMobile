...
export function ThemeSystemProvider({ children, defaultThemeId }: ThemeSystemProviderProps) {
  ...
  const [vscodeTheme, setVSCodeTheme] = useState<Theme | null>(() => {
    ...
    const existing = (window as unknown as { __ORBIT_VSCODE_THEME__?: Theme }).__ORBIT_VSCODE_THEME__;
  ...
  useEffect(() => {
    ...
    window.addEventListener('orbit:theme-hmr', handleThemeHmr);
    return () => window.removeEventListener('orbit:theme-hmr', handleThemeHmr);
  }, []);
  ...
  useEffect(() => {
    ...
    window.addEventListener('orbit:vscode-theme', handleThemeEvent as EventListener);
    return () => window.removeEventListener('orbit:vscode-theme', handleThemeEvent as EventListener);
  }, [isVSCode]);
  ...
  useEffect(() => {
    ...
    const scopedWindow = window as unknown as {
      __orbitApplyThemeSync?: (payload: ThemeSyncPayload) => void;
    };

    scopedWindow.__orbitApplyThemeSync = applyIncomingThemeSync;

    if (receivesParentThemeSync && window.parent !== window) {
      window.parent.postMessage({ type: 'orbit:theme-sync-request' }, window.location.origin);
    }

    return () => {
      if (scopedWindow.__orbitApplyThemeSync === applyIncomingThemeSync) {
        delete scopedWindow.__orbitApplyThemeSync;
      }
    };
  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (data?.type !== 'orbit:theme-sync' || !data.payload) {
        return;
      }
  ...
  useEffect(() => {
    ...
    window.addEventListener('orbit:settings-synced', handleSettingsSynced);
    return () => window.removeEventListener('orbit:settings-synced', handleSettingsSynced);
  }, [receivesParentThemeSync]);
