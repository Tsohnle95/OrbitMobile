...
const MobileShell: React.FC<{ onActiveConnectionDeleted: () => void }> = ({ onActiveConnectionDeleted }) => {
  ...
  const leftResize = useIpadSidebarResize('left', 'orbit.ipad.leftSidebarWidth', IPAD_LEFT_SIDEBAR_WIDTH);
  const rightResize = useIpadSidebarResize(
    'right',
    'orbit.ipad.rightSidebarWidth',
    IPAD_RIGHT_SIDEBAR_WIDTH,
    IPAD_WORKSPACE_SIDEBAR_MAX_WIDTH,
  );
  ...
  // Expose the shell's panel-opening actions to the deep-link layer so orbit:// URLs
...
export function MobileApp({ apis }: MobileAppProps) {
  ...
  const handleNativeResume = React.useCallback(() => {
    ...
    if (now - lastNativeResumeSyncEventAtRef.current >= NATIVE_RESUME_SYNC_EVENT_THROTTLE_MS) {
      ...
      window.dispatchEvent(new Event('orbit:system-resume'));
    }
  ...
  // Single native deep-link entry point: notification taps AND the orbit:// URL
  ...
  if (!fontsReady) {
    return (
      <main className="flex min-h-dvh items-center justify-center bg-background text-foreground">
        <OrbitLogo width={120} height={120} isAnimated />
      </main>
    );
  }
      ...
      return (
        <main className="flex min-h-dvh items-center justify-center bg-background px-6 text-center text-foreground">
          <div className="flex max-w-sm flex-col items-center gap-4">
            <OrbitLogo width={120} height={120} isAnimated={!showConnectionRecovery} />
