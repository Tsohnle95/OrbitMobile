...
const MOBILE_CONNECTIONS_STORAGE_KEY = 'orbit.mobile.connections.v1';
const MOBILE_SECURE_STORAGE_PREFIX = 'orbit.mobile.';
const MOBILE_DEVICE_ID_STORAGE_KEY = 'orbit.mobile.deviceId';
  ...
  const redeemPairingConnection = React.useCallback(async (payload: PairingConnectionPayload) => {
    ...
    try {
      ...
      const redeemBody = JSON.stringify({
        ...
        clientLabel: 'Orbit Mobile',
        clientKind: 'mobile',
        deviceName: 'Orbit Mobile',
  ...
  const submitPassword = React.useCallback(async (password: string) => {
    ...
    try {
      ...
      const loginInit = {
        ...
        body: JSON.stringify({ password, trustDevice: true, issueClientToken: true, clientLabel: 'Orbit Mobile', clientKind: 'mobile', devicePlatform: mobileDevicePlatform(), dedupeKey: mobileClientDedupeKey() }),
      };
