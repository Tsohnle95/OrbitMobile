/**
 * Orbit deep-link vocabulary — the single source of truth for the `orbit://`
...
const segmentsOf = (url: URL): string[] => {
  // Custom-scheme URLs put the first route token in `host` (orbit://session/<id>),
  // but be tolerant of authority-less forms (orbit:/session/<id>) where it lands in
...
/**
 * Parse a raw `orbit://…` string into a typed intent, or `null` if it isn't a
 * recognised Orbit deep link. Tolerant by design: unknown routes return `null`
