...
const MINI_CHAT_PRESENCE_CHANNEL = 'orbit:mini-chat-presence';
...
const MiniChatBootstrap: React.FC<{ config: MiniChatConfig }> = ({ config }) => {
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('orbit:open-session', onOpenSession);
    return () => window.removeEventListener('orbit:open-session', onOpenSession);
