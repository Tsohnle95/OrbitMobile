...
export const applyPersistedHomeDirectoryToWindow = (homeDirectory: string): void => {
  ...
  if (typeof window.__ORBIT_HOME__ === 'string' && window.__ORBIT_HOME__.length > 0) {
    return;
  }

  try {
    window.__ORBIT_HOME__ = homeDirectory;
  } catch {
    /* read-only contextBridge property — leave preload-seeded value */
  }
};

const SETTINGS_MIRROR_INDEX_KEY = 'orbit.settingsMirror.v2.index';
const SETTINGS_MIRROR_KEY_PREFIX = 'orbit.settingsMirror.v2:';
...
const persistToLocalStorage = (settings: DesktopSettings) => {
  ...
  if (typeof settings.pwaAppName === 'string') {
    ...
    if (normalized.length > 0) {
      localStorage.setItem('orbit.pwaName', normalized);
    } else {
      localStorage.removeItem('orbit.pwaName');
    }
  } else {
    localStorage.removeItem('orbit.pwaName');
  }
...
const dispatchSettingsSynced = (settings: DesktopSettings): void => {
  ...
  window.dispatchEvent(new CustomEvent<DesktopSettings>('orbit:settings-synced', { detail: settings }));
};
...
const dispatchSettingsSaveState = (state: 'saving' | 'saved' | 'error'): void => {
  ...
  window.dispatchEvent(new CustomEvent<'saving' | 'saved' | 'error'>('orbit:settings-save-state', { detail: state }));
};
...
export const syncDesktopSettings = async (): Promise<void> => {
  ...
  const applySettings = async (settings: DesktopSettings) => {
    ...
    // `orbit:files:auto-save-enabled`. Prefer the hydrated store value and
