...
export type DesktopSettings = {
  ...
  // User-added skills catalogs (persisted to ~/.config/orbit/settings.json)
...
const getElectronRuntime = (): ElectronRuntimeGlobal | null => {
  ...
  return (window as unknown as { __ORBIT_ELECTRON__?: ElectronRuntimeGlobal }).__ORBIT_ELECTRON__ ?? null;
};

const getDesktopBridge = (): DesktopBridgeGlobal | null => {
  if (typeof window === 'undefined') return null;
  return (window as unknown as { __ORBIT_DESKTOP__?: DesktopBridgeGlobal }).__ORBIT_DESKTOP__ ?? null;
};
...
export const isDesktopLocalOriginActive = (): boolean => {
  ...
  const local = typeof window.__ORBIT_LOCAL_ORIGIN__ === 'string' ? window.__ORBIT_LOCAL_ORIGIN__ : '';
...
export const getDesktopHomeDirectory = async (): Promise<string | null> => {
  if (typeof window !== 'undefined') {
    const embedded = window.__ORBIT_HOME__;
...
export const downloadDesktopUpdate = async (
  ...
  try {
    if (typeof onProgress === 'function' && bridge?.listen) {
      unlisten = await bridge.listen('orbit:update-progress', (evt) => {
