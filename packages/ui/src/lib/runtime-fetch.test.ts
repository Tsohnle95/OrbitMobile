  ...
  test('rewrites current-origin absolute API URLs only', () => {
    ...
    try {
      ...
      Object.defineProperty(globalThis, 'window', {
        configurable: true,
        value: { location: { origin: 'orbit-ui://app', href: 'orbit-ui://app/index.html' } },
      });

      expect(buildRuntimeFetchUrl('orbit-ui://app/api/config/settings')).toBe('https://api.example/api/config/settings');
...
describe('addRuntimeProxyHeaders', () => {
  test('bypasses the ngrok browser interstitial for official ngrok hosts', () => {
    ...
    expect(headers.get('ngrok-skip-browser-warning')).toBe('orbit');
  });
...
describe('runtimeFetch transport contract', () => {
  test('adds the ngrok bypass header to runtime requests', async () => {
    ...
    try {
      ...
      expect(capturedHeaders.get('ngrok-skip-browser-warning')).toBe('orbit');
    } finally {
      setRuntimeUrlResolver(previous);
      globalThis.fetch = originalFetch;
    }
