...
const sessionWith = (linked: unknown): Session =>
  ({ metadata: { orbit: { linked_issues: linked } } } as unknown as Session);
...
describe('withLinkedIssue', () => {
  test('adds a link and preserves unrelated metadata', () => {
    const next = withLinkedIssue(
      { orbit: { kind: 'review' }, other: 1 },
      issue(),
      true,
    );
    expect(next.other).toBe(1);
    expect((next.orbit as Record<string, unknown>).kind).toBe('review');
    expect((next.orbit as { linked_issues: LinkedIssue[] }).linked_issues).toEqual([issue()]);
  });
  ...
  test('re-linking replaces the entry rather than duplicating it', () => {
    ...
    const stored = (second.orbit as { linked_issues: LinkedIssue[] }).linked_issues;
