...
const RUNTIME_ENDPOINT_CHANGED_EVENT = 'orbit:runtime-endpoint-changed';
const RUNTIME_ENDPOINT_WILL_CHANGE_EVENT = 'orbit:runtime-endpoint-will-change';

let activeApiBaseUrl = '';
let activeRuntimeKey = '';

const setWindowRuntimeValue = <K extends '__ORBIT_API_BASE_URL__' | '__ORBIT_CLIENT_TOKEN__' | '__ORBIT_RUNTIME_HEADERS__'>(
...
const readInjectedApiBaseUrl = (): string => {
  ...
  const injected = (window as typeof window & { __ORBIT_API_BASE_URL__?: string }).__ORBIT_API_BASE_URL__;
  return typeof injected === 'string' ? injected.trim() : '';
};

const readInjectedLocalOrigin = (): string => {
  if (typeof window === 'undefined') return '';
  const injected = (window as typeof window & { __ORBIT_LOCAL_ORIGIN__?: string }).__ORBIT_LOCAL_ORIGIN__;
  return typeof injected === 'string' ? injected.trim() : '';
};
...
const readRawRuntimeGlobal = (key: '__ORBIT_API_BASE_URL__' | '__ORBIT_LOCAL_ORIGIN__'): string | undefined => {
...
export const getRuntimeKey = (): string => {
  ...
  const rawApiBaseUrl = readRawRuntimeGlobal('__ORBIT_API_BASE_URL__');
  const rawLocalOrigin = readRawRuntimeGlobal('__ORBIT_LOCAL_ORIGIN__');
  if (
...
export const switchRuntimeEndpoint = (options: { apiBaseUrl: string; clientToken?: string | null; runtimeKey?: string | null; requestHeaders?: Record<string, string> | null; relay?: RelayRuntimeDescriptor | null }): void => {
  ...
  if (typeof window !== 'undefined') {
    ...
    setWindowRuntimeValue(runtimeWindow, '__ORBIT_API_BASE_URL__', apiBaseUrl);
    setWindowRuntimeValue(runtimeWindow, '__ORBIT_CLIENT_TOKEN__', options.clientToken || undefined);
    setWindowRuntimeValue(runtimeWindow, '__ORBIT_RUNTIME_HEADERS__', options.requestHeaders || undefined);
  }
