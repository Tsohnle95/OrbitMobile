...
describe('chatDraftPersistence', () => {
  beforeEach(() => {
    storage.removeItem('orbit.chatDrafts.v2');
  });
  ...
  test('bounds persisted drafts by recency', () => {
    ...
    const envelope = JSON.parse(storage.getItem('orbit.chatDrafts.v2') ?? '{}') as { drafts?: object };
  ...
  test('reuses a parsed envelope while the stored value is unchanged', () => {
    ...
    storage.setItem('orbit.chatDrafts.v2', JSON.stringify({
      version: 2,
