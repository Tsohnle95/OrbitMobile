...
describe('connection payload helpers', () => {
  test('round-trips v2 pairing payloads with direct candidates', () => {
    ...
    expect(encoded.startsWith('orbit://connect?v=2&p=')).toBe(true);
  ...
  test('relay candidate keeps its path and rejects non-ws relay URLs / bad JWKs', () => {
    ...
    expect(parsePairingConnectionPayload(`orbit://connect?v=2&p=${withBadRelay({ type: 'relay', relayUrl: 'https://relay.example/ws', serverId: 'srv', hostEncPubJwk })}`)).toBeNull();
    // Missing serverId.
    expect(parsePairingConnectionPayload(`orbit://connect?v=2&p=${withBadRelay({ type: 'relay', relayUrl: 'wss://relay.example/ws', hostEncPubJwk })}`)).toBeNull();
    // Non-P-256 key.
    expect(parsePairingConnectionPayload(`orbit://connect?v=2&p=${withBadRelay({ type: 'relay', relayUrl: 'wss://relay.example/ws', serverId: 'srv', hostEncPubJwk: { kty: 'EC', crv: 'P-384', x: 'a', y: 'b' } })}`)).toBeNull();
  });
  ...
  test('drops a private-key member from a relay JWK (keeps only public coordinates)', () => {
    ...
    const parsed = parsePairingConnectionPayload(`orbit://connect?v=2&p=${withKey}`);
  ...
  test('rejects invalid v2 pairing payloads', () => {
    expect(parsePairingConnectionPayload('orbit://connect?v=1&server=https://runtime.example&token=t')).toBeNull();
    expect(parsePairingConnectionPayload('orbit://connect?v=2&p=not-json')).toBeNull();
    ...
    expect(parsePairingConnectionPayload(`orbit://connect?v=2&p=${missingSecret}`)).toBeNull();
    ...
    expect(parsePairingConnectionPayload(`orbit://connect?v=2&p=${invalidCandidate}`)).toBeNull();
    ...
    expect(parsePairingConnectionPayload(`orbit://connect?v=2&p=${expired}`)).toBeNull();
  });
});
...
describe('parsePairingConnectionPayloadString (Android WebView fallback)', () => {
  ...
  test('recovers a link whose scheme/host case the URL parser would reject', () => {
    const mixedCase = encoded.replace('orbit://connect', 'Orbit://CONNECT');
  ...
  test('tolerates a trailing slash and reordered query params', () => {
    const trailingSlash = encoded.replace('orbit://connect?', 'orbit://connect/?');
    ...
    expect(parsePairingConnectionPayloadString(`orbit://connect?p=${p}&v=2`)).toEqual(parsePairingConnectionPayload(encoded));
  });
  ...
  test('still rejects non-pairing and malformed payloads', () => {
    ...
    expect(parsePairingConnectionPayloadString('orbit://connect')).toBeNull();
    expect(parsePairingConnectionPayloadString('orbit:///connect?v=2&p=x')).toBeNull();
    expect(parsePairingConnectionPayloadString('orbit://connect?v=1&server=http%3A%2F%2F192.168.1.10%3A2606&token=t')).toBeNull();
    expect(parsePairingConnectionPayloadString('orbit://connect?v=2&p=not-json')).toBeNull();
  });
});
