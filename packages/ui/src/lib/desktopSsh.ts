...
export type DesktopSshInstance = {
  ...
  auth: {
    ...
    orbitPassword?: DesktopSshStoredSecret;
  };
...
const parseInstance = (value: unknown): DesktopSshInstance | null => {
  ...
  const remoteRaw = isRecord(value.remoteOrbit)
    ? value.remoteOrbit
    : isRecord(value.remote_orbit)
      ? value.remote_orbit
      : {};
  ...
  const orbitPassword = parseStoredSecret(authRaw.orbitPassword || authRaw.orbit_password);
...
export const listenDesktopSshStatus = async (
  ...
  const desktop = (window as unknown as { __ORBIT_DESKTOP__?: DesktopBridgeGlobal }).__ORBIT_DESKTOP__;
  const listen = desktop?.listen;
  if (typeof listen !== 'function') {
    return async () => {};
  }

  const unlisten = await listen('orbit:ssh-instance-status', (event) => {
