/**
 * Orbit project-level configuration service.
 * Stores per-project settings in ~/.config/orbit/projects/<projectId>.json.
 * Migrates from legacy <project>/.orbit/orbit.json.
 *
...
const CONFIG_FILENAME = 'orbit.json';
// LEGACY_PROJECT_CONFIG: legacy per-project config root inside repo.
const LEGACY_CONFIG_DIR = '.orbit';
const USER_PROJECTS_DIR_SEGMENTS = ['.config', 'orbit', 'projects'];
...
interface OrbitConfig {
  ...
  projectActions?: OrbitProjectAction[];
  projectActionsPrimaryId?: string;
  draftStarters?: DraftStarterRef[];
}

type OrbitProjectActionPlatform = 'macos' | 'linux' | 'windows';

export interface OrbitProjectAction {
  id: string;
  name: string;
  command: string;
  icon?: string | null;
  platforms?: OrbitProjectActionPlatform[];
  autoOpenUrl?: boolean;
  openUrl?: string;
  desktopOpenSshForward?: string;
}

export interface OrbitProjectActionsState {
  actions: OrbitProjectAction[];
  primaryActionId: string | null;
}

const ORBIT_PROJECT_ACTION_NAME_MAX_LENGTH = 80;
const ORBIT_PROJECT_ACTION_COMMAND_MAX_LENGTH = 4000;
const ORBIT_PROJECT_ACTION_OPEN_URL_MAX_LENGTH = 2000;
const ORBIT_PROJECT_ACTION_DESKTOP_FORWARD_MAX_LENGTH = 300;

const ORBIT_ACTION_PLATFORM_SET = new Set<OrbitProjectActionPlatform>(['macos', 'linux', 'windows']);
...
const resolveHomeDirectory = async (): Promise<string | null> => {
  ...
  // In some runtimes, window.__ORBIT_HOME__ can be workspace/project-root
...
const sanitizeProjectActionPlatforms = (value: unknown): OrbitProjectActionPlatform[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const unique: OrbitProjectActionPlatform[] = [];
  const seen = new Set<OrbitProjectActionPlatform>();
  for (const entry of value) {
    if (typeof entry !== 'string') {
      continue;
    }
    const normalized = entry.trim().toLowerCase() as OrbitProjectActionPlatform;
    if (!ORBIT_ACTION_PLATFORM_SET.has(normalized) || seen.has(normalized)) {
      continue;
    }
    seen.add(normalized);
...
const sanitizeProjectActions = (value: unknown): OrbitProjectAction[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const sanitized: OrbitProjectAction[] = [];
  ...
  for (const entry of value) {
    ...
    const name = trimToMaxLength(typeof record.name === 'string' ? record.name.trim() : '', ORBIT_PROJECT_ACTION_NAME_MAX_LENGTH);
    const command = trimToMaxLength(typeof record.command === 'string' ? record.command.trim() : '', ORBIT_PROJECT_ACTION_COMMAND_MAX_LENGTH);
    ...
    const openUrl = trimToMaxLength(openUrlRaw, ORBIT_PROJECT_ACTION_OPEN_URL_MAX_LENGTH);
    ...
    const desktopOpenSshForward = trimToMaxLength(
      ...
      ORBIT_PROJECT_ACTION_DESKTOP_FORWARD_MAX_LENGTH
    );
...
} | null | undefined): OrbitProjectActionsState => {
 ...
 */
async function readOrbitConfig(project: ProjectRef): Promise<OrbitConfig | null> {
  const projectDirectory = typeof project?.path === 'string' ? project.path.trim() : '';
  if (!projectDirectory) {
    return null;
  }
  ...
  const parseConfig = (text: string | null): OrbitConfig | null => {
  ...
  // 2) Migrate legacy <project>/.orbit/orbit.json.
  // LEGACY_PROJECT_CONFIG: migrate project-local orbit.json -> ~/.config/orbit/projects/<projectId>.json
  ...
  try {
    const wrote = await writeOrbitConfig(project, legacyConfig);
    if (wrote) {
      await deleteLegacyOrbitConfig(projectDirectory);
    }
  } catch {
 ...
 */
async function writeOrbitConfig(
  project: ProjectRef,
  config: OrbitConfig
): Promise<boolean> {
  const projectDirectory = typeof project?.path === 'string' ? project.path.trim() : '';
  ...
  try {
    ...
    console.error('Failed to write orbit config:', error);
    return false;
  }
}
 ...
 */
async function updateOrbitConfig(
  project: ProjectRef,
  updates: Partial<OrbitConfig>
): Promise<boolean> {
  const existing = await readOrbitConfig(project) || {};
  const merged = { ...existing, ...updates };
  return writeOrbitConfig(project, merged);
}
...
export async function getWorktreeSetupCommands(project: ProjectRef): Promise<string[]> {
  const config = await readOrbitConfig(project);
...
export async function saveWorktreeSetupCommands(project: ProjectRef, commands: string[]): Promise<boolean> {
  ...
  return updateOrbitConfig(project, { 'setup-worktree': filtered });
}
...
export async function saveWorktreeSetupWaitEnabled(project: ProjectRef, enabled: boolean): Promise<boolean> {
  return updateOrbitConfig(project, { 'setup-worktree-wait': enabled });
}
...
export async function saveProjectDraftStarters(project: ProjectRef, starters: DraftStarterRef[]): Promise<boolean> {
  return updateOrbitConfig(project, { draftStarters: sanitizeStarterRefs(starters) });
}

export async function getProjectActionsState(project: ProjectRef): Promise<OrbitProjectActionsState> {
  const config = await readOrbitConfig(project);
...
export async function saveProjectActionsState(
  project: ProjectRef,
  value: OrbitProjectActionsState
): Promise<boolean> {
  ...
  return updateOrbitConfig(project, {
...
async function deleteLegacyOrbitConfig(projectDirectory: string): Promise<void> {
  const legacyPath = getLegacyConfigPath(projectDirectory);
  const runtimeFiles = getRuntimeFilesAPI();
