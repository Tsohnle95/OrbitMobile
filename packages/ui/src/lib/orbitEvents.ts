...
type Listener = (event: OrbitEvent) => void;
...
const dispatchFromEnvelope = (envelope: { type: string; properties: unknown }) => {
  if (envelope.type === 'orbit:event-stream-ready') {
    reconnectAttempt = 0;
    return;
  }

  if (envelope.type === 'orbit:heartbeat') {
    return;
  }

  if (envelope.type === 'orbit:agent-memory-changed') {
  ...
  if (envelope.type === 'orbit:session-created') {
    const properties = getEventProperties(envelope.properties);
  ...
  if (envelope.type === 'orbit:browser-control-request') {
    const properties = getEventProperties(envelope.properties);
  ...
  if (envelope.type !== 'orbit:scheduled-task-ran') {
    return;
  }
...
const connect = () => {
  ...
  const canControlBrowser = typeof window !== 'undefined' && Boolean(window.__ORBIT_ELECTRON__);
...
export const subscribeOrbitEvents = (listener: Listener): (() => void) => {
