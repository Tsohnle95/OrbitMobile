...
const ANNOTATION_ACTIVE_FLAG = '__orbitBrowserAnnotationActive';

/** Cancels an overlay left behind by an earlier session. Safe when none is active. */
export const ANNOTATION_TEARDOWN_SCRIPT = `(() => {
  try {
    var host = document.querySelector('[data-orbit-annotation]');
    if (host && host.parentNode) host.parentNode.removeChild(host);
  } catch (error) { /* page navigated away */ }
  try {
    var cursor = document.getElementById('orbit-annotation-cursor');
...
export const buildAnnotationOverlayScript = (
  ...
  var OVERLAY_ATTR = 'data-orbit-annotation';
  ...
  cursorStyle.id = 'orbit-annotation-cursor';
