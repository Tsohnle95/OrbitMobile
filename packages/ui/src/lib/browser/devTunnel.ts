 ...
 * it. When Orbit is running on another host, that is the wrong machine:
...
const isDesktopRuntime = (): boolean => (
  typeof window !== 'undefined' && Boolean(window.__ORBIT_ELECTRON__)
);

/**
 * True when the app is talking to an Orbit on another machine. A local
...
const isRemoteRuntime = (baseUrl: string): boolean => {
  ...
  try {
    ...
    const localOrigin = typeof window !== 'undefined' ? window.__ORBIT_LOCAL_ORIGIN__ : '';
...
/**
 * True when a loopback URL belongs to the machine Orbit runs on rather
 * than to this one.
 *
