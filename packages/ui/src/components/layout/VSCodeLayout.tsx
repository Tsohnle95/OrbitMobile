...
export const VSCodeLayout: React.FC = () => {
  ...
  const [connectionStatus, setConnectionStatus] = React.useState<'connecting' | 'connected' | 'error' | 'disconnected'>(
    () => (typeof window !== 'undefined'
      ? (window as { __ORBIT_CONNECTION__?: { status?: string } }).__ORBIT_CONNECTION__?.status as
  ...
  React.useEffect(() => {
    ...
    void vscodeApi.executeCommand('orbit.setActiveSession', currentSessionId, activeSessionTitle);
  ...
  React.useEffect(() => {
    ...
    void vscodeApi.executeCommand('orbit.updateSessionEditorTitle', currentSessionId, activeSessionTitle);
  ...
  React.useEffect(() => {
    ...
    const current =
      (typeof window !== 'undefined'
        ? (window as { __ORBIT_CONNECTION__?: { status?: string } }).__ORBIT_CONNECTION__?.status
    ...
    window.addEventListener('orbit:connection-status', handler as EventListener);
    return () => window.removeEventListener('orbit:connection-status', handler as EventListener);
  }, []);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('orbit:navigate', handler as EventListener);
    return () => window.removeEventListener('orbit:navigate', handler as EventListener);
  }, []);
    ...
    const runBootstrap = async () => {
      ...
      try {
        const debugEnabled = (() => {
          ...
          try {
            return window.localStorage.getItem('orbit_stream_debug') === '1';
          } catch {
            return false;
          }
        })();

        if (debugEnabled) console.log('[Orbit][VSCode][bootstrap] attempt', { configInitialized });
        ...
        if (debugEnabled) console.log('[Orbit][VSCode][bootstrap] post-load', {
