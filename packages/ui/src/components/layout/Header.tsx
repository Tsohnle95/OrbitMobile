...
export const Header: React.FC = () => {
  ...
  const checkRemoteInstanceUpdate = React.useCallback(async () => {
    ...
    try {
      ...
      const response = await runtimeFetch(`/api/orbit/update-check?${params.toString()}`, {
        method: 'GET',
  ...
  const currentSessionTitle = React.useMemo(() => {
    if (!currentSessionId) {
      return activeProjectLabel ?? 'Orbit';
    }
  ...
  useEffect(() => {
    ...
    void syncFullscreenState();
    window.addEventListener('orbit:window-resized', onResize);

    return () => {
      disposed = true;
      window.removeEventListener('orbit:window-resized', onResize);
    };
