...
export const ContextPanel: React.FC = () => {
  ...
  const postChatSettingsSyncToEmbeddedChat = React.useCallback(() => {
    ...
    for (const frame of chatFrameRefs.current.values()) {
      ...
      frameWindow.postMessage({ type: 'orbit:chat-settings-sync', payload }, window.location.origin);
    }
  }, [allowPromptingSubagentSessions]);
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (data?.type === EMBEDDED_RUNTIME_BOOTSTRAP_REQUEST) {
        ...
        const payload: EmbeddedSessionRuntimeBootstrap = {
          ...
          localOrigin: typeof window.__ORBIT_LOCAL_ORIGIN__ === 'string'
            ? window.__ORBIT_LOCAL_ORIGIN__
            : '',
      ...
      if (data?.type === 'orbit:theme-sync-request') {
        postThemeSyncToEmbeddedChat();
        return;
      }
      if (data?.type === 'orbit:chat-settings-request') {
        postChatSettingsSyncToEmbeddedChat();
        return;
      }
      if (data?.type !== 'orbit:cycle-theme-request') {
        return;
      }
