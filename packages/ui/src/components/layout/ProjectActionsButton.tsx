...
const AUTO_DISCOVER_ACTION_ID = '__orbit_auto_discover_preview__';
...
export const ProjectActionsButton = ({
  ...
  const [actions, setActions] = React.useState<OrbitProjectAction[]>([]);
  ...
  const autoDiscoverAction = React.useMemo<OrbitProjectAction>(() => ({
  ...
  const getOrCreateActionTab = React.useCallback(async (action: OrbitProjectAction, options: { revealTerminal?: boolean } = {}) => {
  ...
  const runAction = React.useCallback(async (action: OrbitProjectAction) => {
    ...
    try {
      const discovered = action.id === AUTO_DISCOVER_ACTION_ID
        ? await (async (): Promise<OrbitProjectAction> => {
  ...
  const stopAction = React.useCallback(async (action: OrbitProjectAction) => {
    const runKey = toProjectActionRunKey(normalizedDirectory, action.id);
  ...
  const handleSelectAction = React.useCallback((action: OrbitProjectAction, toggleStopIfRunning = false) => {
