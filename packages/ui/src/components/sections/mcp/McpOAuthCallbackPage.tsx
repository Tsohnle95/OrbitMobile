...
const returnToApp = (startedFromDesktop: boolean): void => {
  ...
  if (startedFromDesktop) {
    window.location.href = 'orbit://focus/mcp-auth';
    return;
  }
...
const normalizeMcpAuthErrorMessage = (error: unknown, fallback: string): string => {
  ...
  if (/oauth state required/i.test(message)) {
    return 'Authorization session expired or was cleared during reload. Return to Orbit and click Authorize again.';
  }
  return message;
};
...
export const McpOAuthCallbackPage: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      try {
        if (!code) {
          throw new Error('Missing OAuth authorization code. Start authorization again from MCP Settings or paste the returned code into Orbit manually.');
        }
        ...
        if (!pendingContext?.name) {
          throw new Error('Authorization session details were not available. Start authorization again from MCP Settings or paste the returned code into Orbit manually.');
        }
        ...
        setMessage('Authorization completed. You can close this tab and return to Orbit.');
      } catch (authError) {
        if (callbackStateKey) {
          await runtimeFetch(`/api/mcp/auth/pending?state=${encodeURIComponent(callbackStateKey)}`, { method: 'DELETE' }).catch(() => undefined);
        }
