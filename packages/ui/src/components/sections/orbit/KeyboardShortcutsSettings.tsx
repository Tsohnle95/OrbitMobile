...
export const KeyboardShortcutsSettings: React.FC = () => {
  ...
  const actionLabel = React.useCallback((id: string, fallbackLabel: string): string => {
    const key = `settings.orbit.keyboardShortcuts.action.${id}.label`;
  ...
  const saveCombo = React.useCallback((actionId: string, combo: ShortcutCombo) => {
    ...
    setErrorText('');
    setWarningText(isRiskyBrowserShortcut(normalized) ? t('settings.orbit.keyboardShortcuts.warning.riskyBrowserShortcut') : '');
  ...
  const confirmOverwrite = React.useCallback(() => {
    ...
    setPendingOverwrite(null);
    setErrorText('');
    setWarningText(isRiskyBrowserShortcut(pendingOverwrite.combo) ? t('settings.orbit.keyboardShortcuts.warning.riskyBrowserShortcut') : '');
    setDraftByAction((current) => {
      const rest = { ...current };
  ...
  return (
    <SettingsSection
      ...
      title={t('settings.orbit.keyboardShortcuts.title')}
      divider={false}
      info={t('settings.orbit.keyboardShortcuts.tooltip')}
      headerAction={(
        <Button
          type="button"
          variant="outline"
        ...
        >
          {t('settings.orbit.keyboardShortcuts.actions.resetAll')}
        </Button>
      )}
    >
          ...
          {pendingOverwrite && (
            <div className="rounded-lg border border-[var(--status-warning-border)] bg-[var(--status-warning-background)] p-3 flex flex-col @xl:flex-row @xl:items-center justify-between gap-3">
              <span className="typography-meta text-foreground">
                {t('settings.orbit.keyboardShortcuts.overwritePrompt')}
              </span>
              <div className="flex gap-2 shrink-0">
                <Button type="button" size="xs" className="!font-normal" onClick={confirmOverwrite}>{t('settings.orbit.keyboardShortcuts.actions.overwrite')}</Button>
      ...
      <div>
        {actions.map((action, index) => {
          ...
          const displayValue = capturingActionId === action.id
            ? t('settings.orbit.keyboardShortcuts.field.pressKeys')
            : isSurfaceSwitch && !isUnassignedDisplay
              ? `${formatShortcutForDisplay(displayCombo)}${t('settings.orbit.keyboardShortcuts.action.switch_context_surface.suffix')}`
              ...
              >
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    if (!next) {
                      setErrorText(t('settings.orbit.keyboardShortcuts.error.captureFirst'));
                      return;
                    }
