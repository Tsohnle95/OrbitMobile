...
const GITHUB_URL = 'https://github.com/openchamber/orbit';
...
export const AboutSettings: React.FC<AboutSettingsProps> = ({ initialUpdateDialogOpen = false }) => {
  ...
  const [orbitVersion, setOrbitVersion] = React.useState<string | null>(null);
  ...
  const currentVersion = orbitVersion || updateStore.info?.currentVersion || 'unknown';

  React.useEffect(() => {
    let cancelled = false;

    const loadOrbitVersion = async () => {
      try {
        ...
        const data = await response.json().catch(() => null) as { orbitVersion?: unknown } | null;
        const version = typeof data?.orbitVersion === 'string' && data.orbitVersion.trim().length > 0
          ? data.orbitVersion.trim()
          : null;
        if (!cancelled) setOrbitVersion(version);
      } catch {
        if (!cancelled) setOrbitVersion(null);
      }
    };

    void loadOrbitVersion();
    ...
    if (updateStore.checking) {
      ...
      const timer = setTimeout(() => {
        ...
        if (didInitiateCheck.current && !updateStore.available && !updateStore.error) {
          toast.success(t('settings.orbit.about.toast.latestVersion'));
    ...
    return (
      <div className="w-full space-y-6 pb-2">
        <div className="flex flex-col items-center text-center">
          <OrbitLogo width={72} height={72} />
          <h2 className={`mt-4 ${SETTINGS_BRAND_TITLE_CLASS}`}>Orbit</h2>
          <div className="mt-2 space-y-1 typography-ui text-muted-foreground">
            <p>{t('aboutDialog.orbitVersionLabel', { version: currentVersion })}</p>
            <p>{t('aboutDialog.openCodeVersionLabel', { version: openCodeVersion || t('settings.orbit.about.state.unknown') })}</p>
          </div>
          <InstanceServiceUrls />
        </div>
        ...
        <div className="flex justify-center">
          {!updateStore.available && !updateStore.error && (
            ...
            >
              ...
              {isChecking ? t('settings.orbit.about.state.checking') : t('settings.orbit.about.actions.checkForUpdates')}
            </Button>
          )}
          ...
          {!isChecking && updateStore.available && (
            ...
            >
              ...
              {t('settings.orbit.about.actions.updateToVersion', { version: updateStore.info?.version || '' })}
            </Button>
          )}
        </div>
      ...
      <div className="rounded-lg bg-[var(--surface-elevated)]/70 overflow-hidden flex flex-col">
        <div className="flex flex-col @xl:flex-row @xl:items-center justify-between gap-4 px-4 py-3 border-b border-border/40">
          <div className="flex min-w-0 flex-col">
            <span className={SETTINGS_FIELD_LABEL_CLASS}>{t('settings.orbit.about.field.version')}</span>
            <span className="typography-meta text-muted-foreground font-mono">{currentVersion}</span>
          </div>
          <div className="flex min-w-0 flex-col">
            <span className={SETTINGS_FIELD_LABEL_CLASS}>{t('settings.orbit.about.field.openCodeVersion')}</span>
            <span className="typography-meta text-muted-foreground font-mono">{openCodeVersion || t('settings.orbit.about.state.unknown')}</span>
          </div>
          ...
          <div className="flex items-center gap-3">
            {updateStore.checking && (
              <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <span className="typography-meta">{t('settings.orbit.about.state.checking')}</span>
              </div>
            )}
            ...
            {!updateStore.checking && !updateStore.available && !updateStore.error && (
              <span className="typography-meta text-muted-foreground">{t('settings.orbit.about.state.upToDate')}</span>
            )}
            ...
            >
              {t('settings.orbit.about.actions.checkForUpdates')}
            </Button>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2 border-b border-border/40 px-4 py-3 @xl:flex-row @xl:items-center @xl:justify-between">
          <span className={SETTINGS_FIELD_LABEL_CLASS}>{t('settings.orbit.about.field.instanceUrls')}</span>
          <InstanceServiceUrls />
        </div>
