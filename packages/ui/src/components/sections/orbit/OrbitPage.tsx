...
import { OrbitVisualSettings } from './OrbitVisualSettings';
...
import { OrbitToolsSettings } from './OrbitToolsSettings';
...
import type { OrbitSection } from './types';
...
export const OrbitPage: React.FC<OrbitPageProps> = ({ section }) => {
    const { t } = useI18n();
    ...
    if (!section) {
        return (
            <SettingsPageLayout showSaveStatus className="orbit-page-body space-y-3 sm:space-y-6">
                <OrbitVisualSettings />
                <DefaultsSettings />
                {showDesktopNetworkSettings && <DesktopNetworkSettings />}
                {!isVSCode && <OpenCodeCliSettings />}
                {!isVSCode && <OrbitToolsSettings />}
...
const GeneralSectionContent: React.FC = () => {
    ...
    return (
        <>
            ...
            {!isVSCode && <OpenCodeCliSettings />}
            {!isVSCode && <OrbitToolsSettings />}
            <OrbitVisualSettings visibleSettings={[
                'fileEditorKeymap',
...
const VisualSectionContent: React.FC = () => {
    const isVSCode = isVSCodeRuntime();
    return <OrbitVisualSettings visibleSettings={[
        'theme',
