...
export const OpenCodeCliSettings: React.FC = () => {
  ...
  return (
    <SettingsSection title={t('settings.orbit.opencodeCli.title')}>
      <div className="space-y-0.5">
        <SettingsFieldRow
          settingsItem="sessions.opencode-binary"
          label={t('settings.orbit.opencodeCli.field.binaryPath')}
          info={(
            <>
              {t('settings.orbit.opencodeCli.tipPrefix')}
              {' '}
              <span className="font-mono">OPENCODE_BINARY</span>
              {' '}
              {t('settings.orbit.opencodeCli.tipMiddle')}
              {' '}
              <span className="font-mono">~/.config/orbit/settings.json</span>
              {'.'}
            </>
          )}
          alignEnd={false}
        ...
        >
          <Input
            ...
            placeholder={t('settings.orbit.opencodeCli.field.binaryPathPlaceholder')}
          ...
          <Button
            ...
            aria-label={t('settings.orbit.opencodeCli.actions.browseAria')}
            title={t('settings.orbit.opencodeCli.actions.browse')}
          >
        ...
        <SettingsInset className={SETTINGS_OPTION_STACK_CLASS}>
          {!isWindowsArm64() && (
            <SettingsCheckboxRow
              ...
              label={t('settings.orbit.opencodeCli.field.showUpdateNotifications')}
              ariaLabel={t('settings.orbit.opencodeCli.field.showUpdateNotificationsAria')}
            />
          )}
