...
export const DesktopNetworkSettings: React.FC = () => {
  ...
  const isMacDesktop = isLocalDesktop
    ...
    && window.__ORBIT_PLATFORM__ === 'darwin';
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(t('settings.orbit.desktopNetwork.error.loadFailed'));
        }
        ...
        if (!cancelled) {
          setError(cause instanceof Error ? cause.message : t('settings.orbit.desktopNetwork.error.loadFailed'));
        }
      } finally {
        if (!cancelled) {
          setIsLoading(false);
  ...
  const handleLaunchAtLoginToggle = React.useCallback(async () => {
    ...
    try {
      ...
      if (!status?.supported) {
        throw new Error(t('settings.orbit.desktopNetwork.error.launchAtLoginUnsupported'));
      }
      setLaunchAtLoginEnabled(status.enabled);
    } catch (cause) {
      setLaunchAtLoginEnabled(!nextValue);
      setError(cause instanceof Error ? cause.message : t('settings.orbit.desktopNetwork.error.launchAtLoginSaveFailed'));
    } finally {
  ...
  const handleMinimizeToTrayToggle = React.useCallback(async () => {
    ...
    try {
      ...
      if (!status) {
        throw new Error(t('settings.orbit.desktopNetwork.error.minimizeToTraySaveFailed'));
      }
      if (!status.supported) {
        throw new Error(t('settings.orbit.desktopNetwork.error.minimizeToTrayUnsupported'));
      }
      setMinimizeToTrayEnabled(status.enabled);
    } catch (cause) {
      setMinimizeToTrayEnabled(!nextValue);
      setError(cause instanceof Error ? cause.message : t('settings.orbit.desktopNetwork.error.minimizeToTraySaveFailed'));
    } finally {
  ...
  const handleKeepAwakeToggle = React.useCallback(async () => {
    ...
    try {
      ...
      if (!status?.supported) {
        throw new Error(t('settings.orbit.desktopNetwork.error.keepAwakeUnsupported'));
      }
      setKeepAwakeEnabled(status.enabled);
    } catch (cause) {
      setKeepAwakeEnabled(!nextValue);
      setError(cause instanceof Error ? cause.message : t('settings.orbit.desktopNetwork.error.keepAwakeSaveFailed'));
    } finally {
  ...
  const handleSaveAndRestart = React.useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(t('settings.orbit.desktopNetwork.error.saveFailed'));
      }
      ...
      if (!restarted) {
        throw new Error(t('settings.orbit.desktopNetwork.error.savedRestartFailed'));
      }
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : t('settings.orbit.desktopNetwork.error.saveFailed'));
      setIsSaving(false);
    }
  ...
  return (
    <SettingsSection title={t('settings.orbit.desktopNetwork.title')}>
      <div className="space-y-3">
          ...
          <div className={SETTINGS_OPTION_STACK_CLASS}>
            {launchAtLoginSupported ? (
              <SettingsCheckboxRow
                ...
                label={t('settings.orbit.desktopNetwork.field.launchAtLogin')}
                info={t('settings.orbit.desktopNetwork.field.launchAtLoginDescription')}
                ariaLabel={t('settings.orbit.desktopNetwork.field.launchAtLoginAria')}
              />
            ) : null}
            ...
            {isMacDesktop ? (
              <SettingsCheckboxRow
                ...
                label={t('settings.orbit.desktopNetwork.field.macMenuBar')}
                info={t('settings.orbit.desktopNetwork.field.macMenuBarDescription')}
                ariaLabel={t('settings.orbit.desktopNetwork.field.macMenuBarAria')}
              />
            ) : null}
            ...
            {minimizeToTraySupported ? (
              <SettingsCheckboxRow
                ...
                label={t('settings.orbit.desktopNetwork.field.minimizeToTray')}
                info={t('settings.orbit.desktopNetwork.field.minimizeToTrayDescription')}
                ariaLabel={t('settings.orbit.desktopNetwork.field.minimizeToTrayAria')}
              />
            ) : null}
            ...
            {keepAwakeSupported ? (
              <SettingsCheckboxRow
                ...
                label={t('settings.orbit.desktopNetwork.field.keepAwake')}
                info={t('settings.orbit.desktopNetwork.field.keepAwakeDescription')}
                ariaLabel={t('settings.orbit.desktopNetwork.field.keepAwakeAria')}
              />
            ) : null}
          </div>
        ) : null}
        ...
        <SettingsStackedField
          ...
          label={(
            <label htmlFor="desktop-ui-password">
              {t('settings.orbit.desktopPassword.field.password')}
            </label>
          )}
          info={t('settings.orbit.desktopPassword.field.passwordDescription')}
        >
          <Input
            ...
            placeholder={t('settings.orbit.desktopPassword.field.passwordPlaceholder')}
            disabled={isLoading || isSaving}
            required={draftValue}
          ...
          <Button
            ...
            aria-label={t(showPassword ? 'settings.orbit.desktopPassword.actions.hidePassword' : 'settings.orbit.desktopPassword.actions.showPassword')}
        ...
        <div className={SETTINGS_OPTION_STACK_CLASS}>
          <SettingsCheckboxRow
            ...
            disabled={isLoading || isSaving}
            label={t('settings.orbit.desktopNetwork.field.allowLanAccess')}
            info={t('settings.orbit.desktopNetwork.field.allowLanAccessDescription')}
            description={(
              <>
                <span className="block text-[var(--status-warning)]/85">
                  {t('settings.orbit.desktopNetwork.field.warning')}
                </span>
                {lanRequiresPassword || lanBlockedByMissingPassword ? (
                  <span className="block text-[var(--status-warning)]/85">
                    {t('settings.orbit.desktopNetwork.field.passwordRequiredWarning')}
                  </span>
                ) : null}
              </>
            )}
            ariaLabel={t('settings.orbit.desktopNetwork.field.allowLanAccessAria')}
          />
        </div>
        ...
        {lanUrl ? (
          <div className="typography-micro text-muted-foreground/80">
            {isDirty && !savedValue
              ? t('settings.orbit.desktopNetwork.hint.openAfterRestart')
              : t('settings.orbit.desktopNetwork.hint.openNow')}
        ...
        <div className="flex justify-start py-1.5">
          ...
          >
            {isSaving ? t('settings.common.actions.saving') : t('settings.orbit.desktopNetwork.actions.saveAndRestart')}
          </Button>
        </div>
      </div>
    </SettingsSection>
