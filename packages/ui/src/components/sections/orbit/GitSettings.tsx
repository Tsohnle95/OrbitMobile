...
export const GitSettings: React.FC = () => {
  ...
  const viewOptions = React.useMemo(
    () => [
      { id: 'flat' as const, label: t('settings.orbit.git.option.flatList') },
      { id: 'tree' as const, label: t('settings.orbit.git.option.treeView') },
    ],
    [t]
  );
  ...
  return (
    <SettingsSection title={t('settings.orbit.git.title')}>
      <div className={SETTINGS_OPTION_STACK_CLASS}>
        <SettingsControlGroup
          settingsItem="git.changes-view"
          title={t('settings.orbit.git.changesViewTitle')}
        >
          <SettingsRadioGroup aria-label={t('settings.orbit.git.changesViewAria')}>
            {viewOptions.map((option) => (
              <SettingsRadioOption
                ...
                label={option.label}
                ariaLabel={t('settings.orbit.git.optionAria', { option: option.label })}
              />
            ))}
          </SettingsRadioGroup>
        </SettingsControlGroup>
        ...
        <SettingsCheckboxRow
          ...
          label={t('settings.orbit.git.enableGitmoji')}
          ariaLabel={t('settings.orbit.git.enableGitmojiAria')}
        />
        ...
        <SettingsCheckboxRow
          ...
          label={t('settings.orbit.git.showGitignored')}
          ariaLabel={t('settings.orbit.git.showGitignoredAria')}
        />
      </div>
    </SettingsSection>
  );
