...
export const DefaultsSettings: React.FC = () => {
  ...
  const formatVariantLabel = React.useCallback((variant: string) => {
    if (variant === DEFAULT_VARIANT_VALUE) {
      return t('settings.orbit.defaults.option.default');
    }
  ...
  return (
    <>
      <SettingsSection title={t('settings.orbit.defaults.title')} divider={false}>
        <div className="space-y-0">
          <div className="mt-0 mb-1 typography-meta text-muted-foreground">
            {t('settings.orbit.defaults.summaryPrefix')}
            {' '}
            {parsedModel.providerId ? (
              <span className="text-foreground">
                {parsedModel.providerId}/{parsedModel.modelId}
                {supportsVariants ? ` (${defaultVariant ?? t('settings.orbit.defaults.option.defaultLowercase')})` : ''}
              </span>
            ) : (
              <span className="text-foreground">{t('settings.orbit.defaults.summaryOpenCodeDefault')}</span>
            )}
            {defaultAgent && (
              <>
                {' / '}
          ...
          <div>
            <SettingsFieldRow
              ...
              label={t('settings.orbit.defaults.field.defaultModel')}
            >
              <ModelSelector
            ...
            <SettingsFieldRow
              ...
              label={t('settings.orbit.defaults.field.defaultThinking')}
            >
              <Select value={defaultVariant ?? DEFAULT_VARIANT_VALUE} onValueChange={handleVariantChange} disabled={!supportsVariants}>
                <SelectTrigger size={SETTINGS_SELECT_SIZE} className={SETTINGS_SELECT_ROW_TRIGGER_CLASS}>
                  <SelectValue placeholder={t('settings.orbit.defaults.field.thinkingPlaceholder')}>
                    {formatVariantLabel(defaultVariant ?? DEFAULT_VARIANT_VALUE)}
                  </SelectValue>
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value={DEFAULT_VARIANT_VALUE}>{t('settings.orbit.defaults.option.default')}</SelectItem>
            ...
            <SettingsFieldRow
              ...
              label={t('settings.orbit.defaults.field.defaultAgent')}
            >
              <AgentSelector
          ...
          <SettingsInset className={SETTINGS_OPTION_STACK_CLASS}>
            <SettingsCheckboxRow
              ...
              label={t('settings.orbit.defaults.field.showDeletionDialog')}
              ariaLabel={t('settings.orbit.defaults.field.showDeletionDialogAria')}
            />
          </SettingsInset>
          ...
          <div className="space-y-3 pt-6">
            <div className="flex items-center gap-1.5">
              <SettingsGroupTitle>
                {t('settings.orbit.defaults.smallModel.title')}
              </SettingsGroupTitle>
              <SettingsInfoHint>
                {t('settings.orbit.defaults.smallModel.description')}
              </SettingsInfoHint>
            </div>
            ...
            <SettingsCheckboxRow
              ...
              label={t('settings.orbit.defaults.smallModel.useDefault')}
              ariaLabel={t('settings.orbit.defaults.smallModel.useDefaultAria')}
            />

            {!smallModelUseDefault ? (
              <SettingsFieldRow label={t('settings.orbit.defaults.smallModel.overrideModel')}>
                <ModelSelector
            ...
            <SettingsInset className={SETTINGS_OPTION_STACK_CLASS}>
              <div className="flex items-center gap-1.5">
                <SettingsGroupTitle>
                  {t('settings.orbit.defaults.walkthroughModel.title')}
                </SettingsGroupTitle>
                <SettingsInfoHint>
                  {t('settings.orbit.defaults.walkthroughModel.description')}
                </SettingsInfoHint>
              </div>

              <SettingsFieldRow
                settingsItem="sessions.walkthrough-model"
                label={t('settings.orbit.defaults.walkthroughModel.overrideModel')}
              >
                <ModelSelector
                  ...
                  placeholder={t('settings.orbit.defaults.walkthroughModel.usesSmallModel')}
                  className={SETTINGS_CUSTOM_TRIGGER_CLASS}
                />
              </SettingsFieldRow>
            </SettingsInset>
