    ...
    const fetchVersion = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (typeof data.orbitVersion === 'string' && data.orbitVersion.trim()) {
            setVersion(data.orbitVersion);
            return;
          }
        }
      } catch {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xs p-6">
        <div className="flex flex-col items-center text-center space-y-4">
          <OrbitLogo width={64} height={64} />

          <div className="space-y-1">
            <h2 className="text-lg font-semibold">Orbit</h2>
            <div className="space-y-0.5 typography-meta text-muted-foreground">
              {displayVersion && (
                <p>{t('aboutDialog.orbitVersionLabel', { version: displayVersion })}</p>
              )}
              {openCodeVersion && (
          ...
          <p className="typography-micro text-muted-foreground/70 pt-2">
            {t('aboutDialog.attribution')}
          </p>
