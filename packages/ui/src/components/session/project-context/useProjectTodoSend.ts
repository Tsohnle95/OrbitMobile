...
export const useProjectTodoSend = (options: {
  ...
  const confirmSend = React.useCallback(
    async (execution: TodoSendExecution) => {
      ...
      try {
        ...
        if (pendingSendTarget.kind === 'worktree') {
          ...
          initializeNewOrbitSession(session.id, useConfigStore.getState().agents ?? []);
        }
    ...
    [canCreateWorktree, createSession, initializeNewOrbitSession, onActionComplete, pendingSendTarget, projectRef, routeToChat, sendMessage, setCurrentSession, t]
  );
