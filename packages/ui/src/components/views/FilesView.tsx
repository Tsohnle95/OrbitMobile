...
export const FilesView: React.FC<FilesViewProps> = ({ mode = 'full' }) => {
  ...
  const MD_VIEWER_MODE_KEY = 'orbit:files:md-viewer-mode';
  const HTML_VIEWER_MODE_KEY = 'orbit:files:html-viewer-mode';
  const JSON_VIEWER_MODE_KEY = 'orbit:files:json-viewer-mode';
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('orbit:file-viewer-preview-mode-changed', handleFileViewerModeChanged);
    return () => {
      window.removeEventListener('orbit:file-viewer-preview-mode-changed', handleFileViewerModeChanged);
    };
  }, [openPaths]);
