...
import type { OrbitSection } from '@/components/sections/orbit/types';
import { OrbitPage } from '@/components/sections/orbit/OrbitPage';
import { AboutSettings } from '@/components/sections/orbit/AboutSettings';
...
const SETTINGS_DETAIL_HISTORY_KEY = '__orbitSettingsDetail';
...
export const SettingsView: React.FC<SettingsViewProps> = ({ onClose, forceMobile, isWindowed, visiblePageSlugs, initialMobileStage = 'nav' }) => {
  ...
  const isWindows = React.useMemo(() => {
    return isDesktopShell() && typeof window !== 'undefined'
      && (window as unknown as { __ORBIT_PLATFORM__?: string }).__ORBIT_PLATFORM__ === 'win32';
  }, []);
  const isLinux = React.useMemo(() => {
    return isDesktopShell() && typeof window !== 'undefined'
      && (window as unknown as { __ORBIT_PLATFORM__?: string }).__ORBIT_PLATFORM__ === 'linux';
  }, []);
  ...
  const orbitSectionBySlug: Partial<Record<SettingsPageSlug, OrbitSection>> = React.useMemo(() => ({
    general: 'general',
    appearance: 'visual',
    chat: 'chat',
    shortcuts: 'shortcuts',
  ...
  const openSearchResult = React.useCallback((result: SettingsSearchResult) => {
    ...
    if (result.id === 'plugins.create' && typeof window !== 'undefined') {
      window.setTimeout(() => {
        window.dispatchEvent(new CustomEvent('orbit:settings-open-plugin-add'));
      }, 50);
    }
  ...
  const renderPageContent = React.useCallback((slug: SettingsPageSlug) => {
    ...
    switch (slug) {
      ...
      case 'voice':
      case 'tunnel': {
        const section = orbitSectionBySlug[slug] ?? 'visual';
        return <OrbitPage section={section} />;
      }
      case 'home':
      default:
        return null;
    }
  }, [orbitSectionBySlug, openPage, openThirdPartyProviderSetup, renderUnavailable, runtimeCtx, t]);
