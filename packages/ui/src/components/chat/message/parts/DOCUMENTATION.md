...
- The managed `orbit` plugin tool uses the expandable path and hides its broad protocol input. The plugin supplies the selected action's human description as the native tool title; the UI renders that metadata without owning an action map. The full versioned result envelope renders through the same neutral JSON summary/tree/raw views as other tools, without a tool-specific output card.
...
- User-attached context (inline code comments, terminal selections, browser
  ...
  routes to it when the part's metadata carries an `orbitContext`
