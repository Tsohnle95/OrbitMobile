...
const renderMarkdownImageLabel = ({
  ...
  return `<span${titleAttr} class="inline-flex items-center gap-1 align-text-bottom text-muted-foreground" data-orbit-markdown-image-label="true">${escapeAttr(label)}</span>`;
};
...
const createParser = (imageMode: MarkdownImageMode) => new Marked().use({
  ...
  renderer: {
    ...
    link({ href, title, text }) {
      ...
      if (agentName) {
        return `<a href="${escapeAttr(buildAgentMentionUrl(agentName))}" data-orbit-agent-mention="true" class="text-primary hover:underline" target="_blank" rel="noopener noreferrer">${text}</a>`;
      }
...
// (orbit/orbit#2769). Content addressing makes identical blocks
