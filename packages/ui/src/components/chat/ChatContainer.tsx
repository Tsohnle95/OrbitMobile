...
const CHAT_FORCE_SCROLL_BOTTOM_EVENT = 'orbit:chat-force-scroll-bottom';
...
export const ChatContainer: React.FC<ChatContainerProps> = ({
    ...
    React.useEffect(() => {
        ...
        const scopedWindow = window as typeof window & {
            __orbitApplyChatSettingsSync?: (payload: { allowPromptingSubagentSessions: boolean }) => void;
        };
        ...
        const handleMessage = (event: MessageEvent) => {
            ...
            if (data?.type !== 'orbit:chat-settings-sync'
        ...
        scopedWindow.__orbitApplyChatSettingsSync = applySync;
        window.addEventListener('message', handleMessage);
        parentWindow.postMessage({ type: 'orbit:chat-settings-request' }, window.location.origin);
        return () => {
            window.removeEventListener('message', handleMessage);
            if (scopedWindow.__orbitApplyChatSettingsSync === applySync) {
                delete scopedWindow.__orbitApplyChatSettingsSync;
            }
        };
    }, []);
