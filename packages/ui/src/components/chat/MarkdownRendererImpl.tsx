...
const FILE_LINK_SELECTOR = '[data-orbit-file-link="true"]';
const BLOCK_PATH_TOKEN_ATTR = 'data-orbit-block-path-token';
const BLOCK_PATH_TOKEN_SELECTOR = `[${BLOCK_PATH_TOKEN_ATTR}]`;
const CODE_BLOCK_PATH_SCANNED_ATTR = 'data-orbit-block-paths-scanned';
...
// `data-orbit-block-path-token`. `annotateFileLinks` then promotes those
...
// `data-orbit-block-paths-scanned` once processed so the walk is not
...
const useFileReferenceInteractions = ({
  ...
  React.useEffect(() => {
    ...
    const clearFileLinkAttributes = (candidate: HTMLElement) => {
      candidate.removeAttribute('data-orbit-file-link');
      candidate.removeAttribute('data-orbit-file-ref');
      candidate.removeAttribute('data-orbit-file-path');
    ...
    const annotateFileLinks = () => {
      ...
      for (const candidate of Array.from(candidates)) {
        ...
        void existsPromise.then((exists) => {
          ...
          candidate.setAttribute('data-orbit-file-link', 'true');
          candidate.setAttribute('data-orbit-file-ref', latestRawCandidate);
          candidate.setAttribute('data-orbit-file-path', latestResolved.resolvedPath);
    ...
    const openFileReference = async (sourceElement: HTMLElement) => {
      const raw = sourceElement.getAttribute('data-orbit-file-ref') || extractPathCandidateFromElement(sourceElement);
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      const target = event.target;
      if (!(target instanceof HTMLElement) || target.getAttribute('data-orbit-file-link') !== 'true') {
        return;
      }
