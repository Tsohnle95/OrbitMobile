...
// foreign platform's chrome, not Orbit's own status tokens, so the
...
export const WindowsWindowControls = React.memo(function WindowsWindowControls({
  ...
  useEffect(() => {
    ...
    window.addEventListener('orbit:window-maximized-changed', handleMaximizedChange);
    return () => {
      disposed = true;
      window.removeEventListener('orbit:window-maximized-changed', handleMaximizedChange);
    };
  }, [visible]);
