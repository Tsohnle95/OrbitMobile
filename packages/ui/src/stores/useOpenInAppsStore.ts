...
export const useOpenInAppsStore = create<OpenInAppsState>()((set, get) => ({
  ...
  initialize: () => {
    ...
    window.addEventListener('orbit:settings-synced', settingsHandler);
    window.addEventListener('orbit:app-ready', appReadyHandler);
    window.addEventListener('orbit:installed-apps-updated', updateHandler);

    const appReady = (window as unknown as { __orbitAppReady?: boolean }).__orbitAppReady;
    if (appReady) {
      void loadInstalledApps();
    }
