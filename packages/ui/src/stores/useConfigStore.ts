...
const MODELS_DEV_PROXY_URL = "/api/orbit/models-metadata";
...
const fetchOrbitDefaults = async (): Promise<OrbitDefaults> => {
    markStartupTrace('config.defaults:start');
    const started = typeof performance !== 'undefined' ? performance.now() : Date.now();
    const finish = (source: string, result: OrbitDefaults) => {
...
interface ConfigStore {
    ...
    // Orbit settings-based defaults (take precedence over agent preferences)
        ...
        persist(
            (set, get) => ({
                ...
                loadAgents: async (options) => {
                    ...
                    for (let attempt = 0; attempt < 3; attempt++) {
                        try {
                            // Fetch agents and Orbit settings in parallel. OpenCode config
                            ...
                            const [agents, orbitDefaults] = await Promise.all([
                                measureStartupTrace(
                                    'loadAgents:api',
                            ...
                            const defaultZenModel = normalizeOptionalString(orbitDefaults.zenModel);
                            ...
                            set((state) => {
                                ...
                                const nextState: Partial<ConfigStore> = {
                                    settingsDefaultModel: orbitDefaults.defaultModel,
                                    settingsDefaultVariant: orbitDefaults.defaultVariant,
                                    settingsDefaultAgent: orbitDefaults.defaultAgent,
                                    settingsAutoCreateWorktree: orbitDefaults.autoCreateWorktree ?? false,
                                    settingsGitmojiEnabled: orbitDefaults.gitmojiEnabled ?? false,
                                    settingsDefaultFileViewerPreview: orbitDefaults.defaultFileViewerPreview ?? false,
                                    settingsZenModel: resolvedZenModel,
                                    settingsMessageStreamTransport: orbitDefaults.messageStreamTransport ?? state.settingsMessageStreamTransport ?? 'auto',
                                    sttProvider: orbitDefaults.sttProvider ?? state.sttProvider,
                                    sttServerUrl: orbitDefaults.sttServerUrl ?? state.sttServerUrl,
                                    sttModel: orbitDefaults.sttModel ?? state.sttModel,
                                    sttLocalModel: orbitDefaults.sttLocalModel ?? state.sttLocalModel,
                                    sttLanguage: orbitDefaults.sttLanguage ?? state.sttLanguage,
                                    directoryScoped: {
                                        ...state.directoryScoped,
                                        [directoryKey]: nextSnapshot,
                                    },
                            ...
                            // Detect invalid Orbit settings so we can clear them from storage.
                            ...
                            if (orbitDefaults.defaultAgent && !safeAgents.some((agent) => agent.name === orbitDefaults.defaultAgent)) {
                                invalidSettings.defaultAgent = '';
                            }
                            if (orbitDefaults.defaultModel) {
                                const parsed = parseModelString(orbitDefaults.defaultModel);
                                if (!parsed || !validateModel(parsed.providerId, parsed.modelId)) {
                                    invalidSettings.defaultModel = '';
                                } else if (orbitDefaults.defaultVariant) {
                                    const provider = providers.find((p) => p.id === parsed.providerId);
                                    const model = provider?.models.find((m) => m.id === parsed.modelId) as { variants?: Record<string, unknown> } | undefined;
                                    const variants = model?.variants;
                                    if (!(variants && Object.prototype.hasOwnProperty.call(variants, orbitDefaults.defaultVariant))) {
                ...
                setAgent: (agentName: string | undefined) => {
                    ...
                    if (agentName) {
                        ...
                        if (currentSessionId && useSessionUIStore.getState().isOrbitCreatedSession(currentSessionId)) {
                            const existingAgentModel = selState.getAgentModelForSession(currentSessionId, agentName);
                            if (!existingAgentModel) {
                                useSessionUIStore.getState().initializeNewOrbitSession(currentSessionId, agents);
                            }
                        }
                    }
