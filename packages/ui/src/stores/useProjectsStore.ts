...
const getLocalRuntimeOrigin = (): string => {
  ...
  const value = (window as typeof window & { __ORBIT_LOCAL_ORIGIN__?: string }).__ORBIT_LOCAL_ORIGIN__;
...
const createVSCodeWorkspaceProjects = (
  ...
  if (streamDebugEnabled()) {
    console.log('[Orbit][VSCode][projects] Using workspace projects', projects);
  }
...
if (typeof window !== 'undefined') {
  window.addEventListener('orbit:settings-synced', (event: Event) => {
