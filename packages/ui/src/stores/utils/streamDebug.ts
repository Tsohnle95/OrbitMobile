...
const STREAM_PERF_STORAGE_KEY = 'orbit_stream_perf';
...
declare global {
    interface Window {
        __orbitStreamPerfState?: StreamPerfState;
        __orbitStreamPerformance?: {
            setEnabled: (enabled: boolean) => void;
            reset: () => void;
            getSnapshot: () => StreamPerfSnapshot;
        };
        __orbitVsCodeStreamPerfState?: {
            counters: Map<string, PerfCounter>;
            lastReportAt?: number;
            lastUpdatedAt?: number;
...
const ensureStreamPerfState = (): StreamPerfState | null => {
    ...
    if (!window.__orbitStreamPerfState) {
        const startedAt = Date.now();
        window.__orbitStreamPerfState = {
    ...
    return window.__orbitStreamPerfState;
};
...
export const setStreamPerfEnabled = (enabled: boolean): void => {
    ...
    try {
        ...
        delete window.__orbitStreamPerfState;
        delete window.__orbitVsCodeStreamPerfState;
    } catch {
...
export const resetStreamPerf = (): void => {
    ...
    if (window.__orbitVsCodeStreamPerfState) {
        window.__orbitVsCodeStreamPerfState = {
            ...window.__orbitVsCodeStreamPerfState,
            counters: new Map<string, PerfCounter>(),
            startedAt: Date.now(),
            lastUpdatedAt: Date.now(),
...
export const getStreamPerfSnapshot = (): StreamPerfSnapshot => {
    ...
    const state = window.__orbitStreamPerfState;
...
export const getVsCodeStreamPerfSnapshot = (): StreamPerfSnapshot => {
    ...
    const state = window.__orbitVsCodeStreamPerfState;
    if (!streamPerfEnabled || !state) {
        return {
            enabled: false,
            startedAt: null,
...
if (typeof window !== 'undefined') {
    window.__orbitStreamPerformance = {
