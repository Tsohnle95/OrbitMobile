const STORAGE_KEY = "orbit_session_load_perf"
...
declare global {
  interface Window {
    __orbitSessionLoadPerformance?: SessionLoadPerformanceState
  }
}
...
export function startSessionLoadPerformanceEvent(input: Omit<SessionLoadPerformanceEvent, "at" | "durationMs" | "outcome">) {
  ...
  return (
    ...
    const state = window.__orbitSessionLoadPerformance ?? { events: [] }
    ...
    window.__orbitSessionLoadPerformance = state
  }
}
...
const FIRST_VISIBLE_MARK = "orbit.chat.first_message_visible"
