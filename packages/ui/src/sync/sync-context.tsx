...
const SYNC_CONTEXT_GLOBAL_KEY = "__orbit_sync_context__"
...
const handleUiNotificationEvent = (payload: Event, fallbackDirectory: string): boolean => {
  if ((payload as { type?: unknown }).type !== "orbit:notification") {
    return false
  }
...
const dispatchVSCodeRuntimeNotificationEvent = (directory: string, payload: Event) => {
  ...
  window.dispatchEvent(new CustomEvent("orbit:vscode-notification-event", {
...
export function handleEvent(
  ...
  if ((payload as { type?: unknown }).type === "orbit:permission-auto-accept.updated") {
...
const dispatchOpenCodeUpdateAvailable = (payload: { version: string }) => {
  ...
  window.dispatchEvent(new CustomEvent("orbit:opencode-update-available", { detail: payload }))
}
...
export function SyncProvider(props: {
  ...
  useEffect(() => {
    ...
    window.addEventListener("orbit:system-resume", onSystemResume)
    return () => window.removeEventListener("orbit:system-resume", onSystemResume)
  }, [childStores])
