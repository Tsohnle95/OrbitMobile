...
const normalizeOrbitSessionStatus = (payload: Event): Event | null => {
  ...
  if (record.type !== "orbit:session-status") return null
  ...
  return {
    id: typeof record.id === "string" && record.id.length > 0
      ? record.id
      : `orbit-status-${sessionID}-${Date.now()}`,
    type: "session.status",
    properties: {
      sessionID,
      status,
...
const normalizeEventType = (payload: Event): Event => {
  const normalizedOrbitStatus = normalizeOrbitSessionStatus(payload)
  if (normalizedOrbitStatus) {
    return normalizedOrbitStatus
  }
...
export function createEventPipeline(input: EventPipelineInput): EventPipeline {
  ...
  if (typeof globalThis.window !== "undefined") {
    globalThis.window.addEventListener("orbit:system-resume", onSystemResume)
  ...
  const cleanup = () => {
    ...
    if (typeof globalThis.window !== "undefined") {
      globalThis.window.removeEventListener("orbit:system-resume", onSystemResume)
