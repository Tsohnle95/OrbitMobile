...
describe("SessionMessageLoader", () => {
  ...
  test("reports retries and every downloaded initial expansion record", async () => {
    ...
    const diagnosticWindow = {
      location: { search: "" },
      localStorage: {
        getItem: (key: string) => key === "orbit_session_load_perf" ? "1" : null,
      },
    ...
    try {
      ...
      const events = diagnosticWindow.__orbitSessionLoadPerformance?.events ?? []
...
describe("session load performance diagnostics", () => {
  test("rejects unknown raw labels and preserves approved input counts", () => {
    ...
    try {
      ...
      expect(diagnosticWindow.__orbitSessionLoadPerformance?.events).toHaveLength(1)
      const event = diagnosticWindow.__orbitSessionLoadPerformance?.events[0]
      ...
      expect(JSON.stringify(diagnosticWindow.__orbitSessionLoadPerformance)).not.toContain("secret")
    } finally {
      if (originalWindow) Object.defineProperty(globalThis, "window", originalWindow)
      else Reflect.deleteProperty(globalThis, "window")
    }
