...
Session loading instrumentation is disabled by default. Set `localStorage.orbit_session_load_perf` to `"1"`, reproduce the interaction, then inspect `window.__orbitSessionLoadPerformance.events`.
...
High-frequency sync diagnostics are separately disabled by default. Set `localStorage.orbit_sync_perf` to `"1"` before reload to enable fixed numeric counters for pipeline traffic, reducer publications, streaming reconciliations, entries/messages visited, targeted heartbeat work, and persistence serialization/write volume. The hot path performs only a null check while disabled; counters never retain IDs, payloads, or user content.

Browser profiling also enables `localStorage.orbit_stream_perf` to capture bounded aggregate timings and render counts for chat projections, message components, and major sidebar boundaries. These metrics contain no session IDs or user content and are reset immediately before each recording.
