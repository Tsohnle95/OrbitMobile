...
const markSessionAsOrbitCreatedCalls: string[] = []
...
mock.module("../session-ui-store", () => ({
  useSessionUIStore: {
    getState: () => ({
      ...
      markSessionAsOrbitCreated: (sessionId: string) => {
        markSessionAsOrbitCreatedCalls.push(sessionId)
      },
    }),
  },
}))
...
beforeEach(() => {
  ...
  markSessionAsOrbitCreatedCalls.length = 0
