...
import { getWorktreeSetupWaitEnabled } from "@/lib/orbitConfig"
...
export type SessionUIState = {
  ...
  clearError: () => void
  markSessionAsOrbitCreated: (sessionId: string) => void
  isOrbitCreatedSession: (sessionId: string) => boolean
  getContextUsage: (contextLimit: number, outputLimit: number) => SessionContextUsage | null
  initializeNewOrbitSession: (sessionId: string, agents: unknown[]) => void
...
export async function materializeOpenDraftSession(selection: {
  ...
  const created = await createSessionWithDraftLifecycle(
    ...
    draftPins.notes.length > 0 || draftPins.plans.length > 0
      ? { orbit: { project_context_pins: draftPins } }
      : undefined,
    "submitted-draft",
  )
  if (!created?.id) {
  ...
  store.initializeNewOrbitSession(created.id, configState.agents ?? [])
...
export const useSessionUIStore = create<SessionUIState>()((set, get) => ({
  ...
  markSessionAsOrbitCreated: (sessionId) =>
    set((s) => {
  ...
  isOrbitCreatedSession: (sessionId) => get().webUICreatedSessions.has(sessionId),
