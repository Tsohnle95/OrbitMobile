...
function App({ apis }: AppProps) {
  React.useEffect(() => {
    ...
    if (startupTraceEnabled()) {
      console.info('[startup-trace] enabled. Run console.table(window.__ORBIT_STARTUP_TRACE__) after startup.');
    }
  }, []);
  ...
  React.useEffect(() => {
    ...
    const scopedWindow = window as unknown as {
      __orbitSetEmbeddedVisibility?: (payload?: EmbeddedVisibilityPayload) => void;
    };

    scopedWindow.__orbitSetEmbeddedVisibility = applyVisibility;
    window.addEventListener('message', handleMessage);
    requestEmbeddedSessionVisibility();

    return () => {
      window.removeEventListener('message', handleMessage);
      if (scopedWindow.__orbitSetEmbeddedVisibility === applyVisibility) {
        delete scopedWindow.__orbitSetEmbeddedVisibility;
      }
    };
  }, [embeddedSessionChat]);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('orbit:open-session', handler as EventListener);
    return () => window.removeEventListener('orbit:open-session', handler as EventListener);
  }, []);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('orbit:open-mini-chat', onOpenMiniChat);
    return () => window.removeEventListener('orbit:open-mini-chat', onOpenMiniChat);
  }, []);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('orbit:open-draft-session', handler as EventListener);
    return () => window.removeEventListener('orbit:open-draft-session', handler as EventListener);
  }, []);
  ...
  React.useEffect(() => {
    ...
    (window as unknown as { __orbitAppReady?: boolean }).__orbitAppReady = true;
    window.dispatchEvent(new Event('orbit:app-ready'));
  ...
  // The Rust backend sets window.__ORBIT_DESKTOP_BOOT_OUTCOME__ once the
